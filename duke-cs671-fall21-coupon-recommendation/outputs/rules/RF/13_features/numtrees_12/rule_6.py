def findDecision(obj): #obj[0]: Passanger, obj[1]: Time, obj[2]: Coupon, obj[3]: Gender, obj[4]: Age, obj[5]: Children, obj[6]: Education, obj[7]: Occupation, obj[8]: Bar, obj[9]: Coffeehouse, obj[10]: Restaurant20to50, obj[11]: Direction_same, obj[12]: Distance
	# {"feature": "Bar", "instances": 85, "metric_value": 0.9879, "depth": 1}
	if obj[8]<=2.0:
		# {"feature": "Education", "instances": 71, "metric_value": 0.9477, "depth": 2}
		if obj[6]<=3:
			# {"feature": "Restaurant20to50", "instances": 66, "metric_value": 0.9673, "depth": 3}
			if obj[10]>-1.0:
				# {"feature": "Coffeehouse", "instances": 64, "metric_value": 0.9544, "depth": 4}
				if obj[9]>1.0:
					# {"feature": "Occupation", "instances": 32, "metric_value": 0.8113, "depth": 5}
					if obj[7]>1:
						# {"feature": "Age", "instances": 26, "metric_value": 0.8905, "depth": 6}
						if obj[4]<=5:
							# {"feature": "Coupon", "instances": 24, "metric_value": 0.8113, "depth": 7}
							if obj[2]>0:
								# {"feature": "Direction_same", "instances": 21, "metric_value": 0.7025, "depth": 8}
								if obj[11]<=0:
									# {"feature": "Time", "instances": 15, "metric_value": 0.8366, "depth": 9}
									if obj[1]>1:
										# {"feature": "Children", "instances": 9, "metric_value": 0.5033, "depth": 10}
										if obj[5]<=0:
											return 'True'
										elif obj[5]>0:
											# {"feature": "Passanger", "instances": 4, "metric_value": 0.8113, "depth": 11}
											if obj[0]>2:
												# {"feature": "Gender", "instances": 3, "metric_value": 0.9183, "depth": 12}
												if obj[3]>0:
													# {"feature": "Distance", "instances": 2, "metric_value": 1.0, "depth": 13}
													if obj[12]<=2:
														return 'False'
													else: return 'False'
												elif obj[3]<=0:
													return 'True'
												else: return 'True'
											elif obj[0]<=2:
												return 'True'
											else: return 'True'
										else: return 'True'
									elif obj[1]<=1:
										# {"feature": "Passanger", "instances": 6, "metric_value": 1.0, "depth": 10}
										if obj[0]<=1:
											# {"feature": "Children", "instances": 5, "metric_value": 0.971, "depth": 11}
											if obj[5]<=0:
												# {"feature": "Gender", "instances": 4, "metric_value": 0.8113, "depth": 12}
												if obj[3]<=0:
													# {"feature": "Distance", "instances": 3, "metric_value": 0.9183, "depth": 13}
													if obj[12]<=1:
														return 'False'
													else: return 'False'
												elif obj[3]>0:
													return 'False'
												else: return 'False'
											elif obj[5]>0:
												return 'True'
											else: return 'True'
										elif obj[0]>1:
											return 'True'
										else: return 'True'
									else: return 'True'
								elif obj[11]>0:
									return 'True'
								else: return 'True'
							elif obj[2]<=0:
								# {"feature": "Passanger", "instances": 3, "metric_value": 0.9183, "depth": 8}
								if obj[0]>0:
									return 'False'
								elif obj[0]<=0:
									return 'True'
								else: return 'True'
							else: return 'False'
						elif obj[4]>5:
							return 'False'
						else: return 'False'
					elif obj[7]<=1:
						return 'True'
					else: return 'True'
				elif obj[9]<=1.0:
					# {"feature": "Occupation", "instances": 32, "metric_value": 1.0, "depth": 5}
					if obj[7]<=15:
						# {"feature": "Age", "instances": 27, "metric_value": 0.9911, "depth": 6}
						if obj[4]>1:
							# {"feature": "Coupon", "instances": 19, "metric_value": 0.998, "depth": 7}
							if obj[2]>0:
								# {"feature": "Direction_same", "instances": 16, "metric_value": 0.9887, "depth": 8}
								if obj[11]<=0:
									# {"feature": "Time", "instances": 15, "metric_value": 0.971, "depth": 9}
									if obj[1]>1:
										# {"feature": "Children", "instances": 12, "metric_value": 1.0, "depth": 10}
										if obj[5]>0:
											# {"feature": "Gender", "instances": 6, "metric_value": 0.9183, "depth": 11}
											if obj[3]>0:
												# {"feature": "Passanger", "instances": 3, "metric_value": 0.9183, "depth": 12}
												if obj[0]>1:
													return 'False'
												elif obj[0]<=1:
													return 'True'
												else: return 'True'
											elif obj[3]<=0:
												return 'True'
											else: return 'True'
										elif obj[5]<=0:
											# {"feature": "Passanger", "instances": 6, "metric_value": 0.9183, "depth": 11}
											if obj[0]<=0:
												# {"feature": "Distance", "instances": 3, "metric_value": 0.9183, "depth": 12}
												if obj[12]<=1:
													return 'True'
												elif obj[12]>1:
													return 'False'
												else: return 'False'
											elif obj[0]>0:
												return 'False'
											else: return 'False'
										else: return 'False'
									elif obj[1]<=1:
										return 'True'
									else: return 'True'
								elif obj[11]>0:
									return 'False'
								else: return 'False'
							elif obj[2]<=0:
								return 'False'
							else: return 'False'
						elif obj[4]<=1:
							# {"feature": "Time", "instances": 8, "metric_value": 0.8113, "depth": 7}
							if obj[1]>0:
								return 'True'
							elif obj[1]<=0:
								return 'False'
							else: return 'False'
						else: return 'True'
					elif obj[7]>15:
						# {"feature": "Time", "instances": 5, "metric_value": 0.7219, "depth": 6}
						if obj[1]>0:
							return 'False'
						elif obj[1]<=0:
							return 'True'
						else: return 'True'
					else: return 'False'
				else: return 'False'
			elif obj[10]<=-1.0:
				return 'False'
			else: return 'False'
		elif obj[6]>3:
			return 'True'
		else: return 'True'
	elif obj[8]>2.0:
		# {"feature": "Occupation", "instances": 14, "metric_value": 0.7496, "depth": 2}
		if obj[7]<=17:
			# {"feature": "Passanger", "instances": 12, "metric_value": 0.4138, "depth": 3}
			if obj[0]<=2:
				return 'False'
			elif obj[0]>2:
				# {"feature": "Gender", "instances": 3, "metric_value": 0.9183, "depth": 4}
				if obj[3]<=0:
					return 'False'
				elif obj[3]>0:
					return 'True'
				else: return 'True'
			else: return 'False'
		elif obj[7]>17:
			return 'True'
		else: return 'True'
	else: return 'False'
