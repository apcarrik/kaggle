def findDecision(obj): #obj[0]: Passanger, obj[1]: Weather, obj[2]: Time, obj[3]: Coupon, obj[4]: Coupon_validity, obj[5]: Gender, obj[6]: Age, obj[7]: Children, obj[8]: Education, obj[9]: Occupation, obj[10]: Income, obj[11]: Bar, obj[12]: Coffeehouse, obj[13]: Restaurant20to50, obj[14]: Direction_same, obj[15]: Distance
	# {"feature": "Coupon", "instances": 85, "metric_value": 0.9951, "depth": 1}
	if obj[3]>1:
		# {"feature": "Education", "instances": 58, "metric_value": 0.9294, "depth": 2}
		if obj[8]>1:
			# {"feature": "Coffeehouse", "instances": 32, "metric_value": 1.0, "depth": 3}
			if obj[12]<=3.0:
				# {"feature": "Distance", "instances": 26, "metric_value": 0.9612, "depth": 4}
				if obj[15]<=2:
					# {"feature": "Income", "instances": 22, "metric_value": 0.994, "depth": 5}
					if obj[10]<=4:
						# {"feature": "Bar", "instances": 13, "metric_value": 0.9612, "depth": 6}
						if obj[11]<=2.0:
							# {"feature": "Occupation", "instances": 11, "metric_value": 0.8454, "depth": 7}
							if obj[9]<=9:
								# {"feature": "Time", "instances": 7, "metric_value": 0.9852, "depth": 8}
								if obj[2]>1:
									# {"feature": "Children", "instances": 5, "metric_value": 0.7219, "depth": 9}
									if obj[7]<=0:
										return 'True'
									elif obj[7]>0:
										# {"feature": "Weather", "instances": 2, "metric_value": 1.0, "depth": 10}
										if obj[1]<=0:
											return 'False'
										elif obj[1]>0:
											return 'True'
										else: return 'True'
									else: return 'False'
								elif obj[2]<=1:
									return 'False'
								else: return 'False'
							elif obj[9]>9:
								return 'True'
							else: return 'True'
						elif obj[11]>2.0:
							return 'False'
						else: return 'False'
					elif obj[10]>4:
						# {"feature": "Coupon_validity", "instances": 9, "metric_value": 0.7642, "depth": 6}
						if obj[4]>0:
							return 'False'
						elif obj[4]<=0:
							# {"feature": "Passanger", "instances": 3, "metric_value": 0.9183, "depth": 7}
							if obj[0]<=1:
								return 'True'
							elif obj[0]>1:
								return 'False'
							else: return 'False'
						else: return 'True'
					else: return 'False'
				elif obj[15]>2:
					return 'False'
				else: return 'False'
			elif obj[12]>3.0:
				return 'True'
			else: return 'True'
		elif obj[8]<=1:
			# {"feature": "Age", "instances": 26, "metric_value": 0.6194, "depth": 3}
			if obj[6]>2:
				# {"feature": "Occupation", "instances": 19, "metric_value": 0.2975, "depth": 4}
				if obj[9]<=10:
					return 'True'
				elif obj[9]>10:
					# {"feature": "Weather", "instances": 5, "metric_value": 0.7219, "depth": 5}
					if obj[1]>0:
						return 'True'
					elif obj[1]<=0:
						# {"feature": "Passanger", "instances": 2, "metric_value": 1.0, "depth": 6}
						if obj[0]>1:
							return 'True'
						elif obj[0]<=1:
							return 'False'
						else: return 'False'
					else: return 'True'
				else: return 'True'
			elif obj[6]<=2:
				# {"feature": "Coupon_validity", "instances": 7, "metric_value": 0.9852, "depth": 4}
				if obj[4]>0:
					# {"feature": "Passanger", "instances": 4, "metric_value": 0.8113, "depth": 5}
					if obj[0]>1:
						# {"feature": "Occupation", "instances": 2, "metric_value": 1.0, "depth": 6}
						if obj[9]>10:
							return 'False'
						elif obj[9]<=10:
							return 'True'
						else: return 'True'
					elif obj[0]<=1:
						return 'False'
					else: return 'False'
				elif obj[4]<=0:
					return 'True'
				else: return 'True'
			else: return 'True'
		else: return 'True'
	elif obj[3]<=1:
		# {"feature": "Bar", "instances": 27, "metric_value": 0.8767, "depth": 2}
		if obj[11]>0.0:
			# {"feature": "Income", "instances": 14, "metric_value": 1.0, "depth": 3}
			if obj[10]<=4:
				# {"feature": "Passanger", "instances": 10, "metric_value": 0.8813, "depth": 4}
				if obj[0]<=1:
					# {"feature": "Occupation", "instances": 8, "metric_value": 0.5436, "depth": 5}
					if obj[9]>1:
						return 'False'
					elif obj[9]<=1:
						return 'True'
					else: return 'True'
				elif obj[0]>1:
					return 'True'
				else: return 'True'
			elif obj[10]>4:
				return 'True'
			else: return 'True'
		elif obj[11]<=0.0:
			# {"feature": "Coupon_validity", "instances": 13, "metric_value": 0.3912, "depth": 3}
			if obj[4]<=0:
				return 'False'
			elif obj[4]>0:
				# {"feature": "Passanger", "instances": 2, "metric_value": 1.0, "depth": 4}
				if obj[0]<=1:
					return 'True'
				elif obj[0]>1:
					return 'False'
				else: return 'False'
			else: return 'True'
		else: return 'False'
	else: return 'False'
