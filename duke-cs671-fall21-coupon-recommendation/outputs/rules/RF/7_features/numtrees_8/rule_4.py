def findDecision(obj): #obj[0]: Passanger, obj[1]: Coupon, obj[2]: Education, obj[3]: Occupation, obj[4]: Restaurant20to50, obj[5]: Direction_same, obj[6]: Distance
	# {"feature": "Passanger", "instances": 127, "metric_value": 0.9964, "depth": 1}
	if obj[0]<=2:
		# {"feature": "Distance", "instances": 93, "metric_value": 0.9959, "depth": 2}
		if obj[6]<=1:
			# {"feature": "Occupation", "instances": 54, "metric_value": 0.9911, "depth": 3}
			if obj[3]<=14:
				# {"feature": "Restaurant20to50", "instances": 51, "metric_value": 0.9975, "depth": 4}
				if obj[4]<=3.0:
					# {"feature": "Direction_same", "instances": 49, "metric_value": 0.9925, "depth": 5}
					if obj[5]<=0:
						# {"feature": "Coupon", "instances": 31, "metric_value": 0.9992, "depth": 6}
						if obj[1]<=3:
							# {"feature": "Education", "instances": 22, "metric_value": 0.976, "depth": 7}
							if obj[2]<=1:
								return 'False'
							elif obj[2]>1:
								return 'True'
							else: return 'True'
						elif obj[1]>3:
							# {"feature": "Education", "instances": 9, "metric_value": 0.7642, "depth": 7}
							if obj[2]<=2:
								return 'False'
							elif obj[2]>2:
								return 'False'
							else: return 'False'
						else: return 'False'
					elif obj[5]>0:
						# {"feature": "Education", "instances": 18, "metric_value": 0.9183, "depth": 6}
						if obj[2]>0:
							# {"feature": "Coupon", "instances": 14, "metric_value": 0.9852, "depth": 7}
							if obj[1]>1:
								return 'False'
							elif obj[1]<=1:
								return 'True'
							else: return 'True'
						elif obj[2]<=0:
							return 'True'
						else: return 'True'
					else: return 'True'
				elif obj[4]>3.0:
					return 'False'
				else: return 'False'
			elif obj[3]>14:
				return 'True'
			else: return 'True'
		elif obj[6]>1:
			# {"feature": "Coupon", "instances": 39, "metric_value": 0.9183, "depth": 3}
			if obj[1]<=3:
				# {"feature": "Occupation", "instances": 30, "metric_value": 0.9871, "depth": 4}
				if obj[3]>2:
					# {"feature": "Education", "instances": 23, "metric_value": 0.9321, "depth": 5}
					if obj[2]<=2:
						# {"feature": "Restaurant20to50", "instances": 17, "metric_value": 0.9975, "depth": 6}
						if obj[4]>0.0:
							# {"feature": "Direction_same", "instances": 16, "metric_value": 1.0, "depth": 7}
							if obj[5]<=0:
								return 'True'
							elif obj[5]>0:
								return 'False'
							else: return 'False'
						elif obj[4]<=0.0:
							return 'False'
						else: return 'False'
					elif obj[2]>2:
						return 'False'
					else: return 'False'
				elif obj[3]<=2:
					# {"feature": "Education", "instances": 7, "metric_value": 0.8631, "depth": 5}
					if obj[2]<=1:
						# {"feature": "Direction_same", "instances": 5, "metric_value": 0.971, "depth": 6}
						if obj[5]<=0:
							# {"feature": "Restaurant20to50", "instances": 4, "metric_value": 1.0, "depth": 7}
							if obj[4]<=1.0:
								return 'True'
							else: return 'True'
						elif obj[5]>0:
							return 'True'
						else: return 'True'
					elif obj[2]>1:
						return 'True'
					else: return 'True'
				else: return 'True'
			elif obj[1]>3:
				return 'False'
			else: return 'False'
		else: return 'False'
	elif obj[0]>2:
		# {"feature": "Occupation", "instances": 34, "metric_value": 0.8338, "depth": 2}
		if obj[3]<=12:
			# {"feature": "Education", "instances": 29, "metric_value": 0.8936, "depth": 3}
			if obj[2]>0:
				# {"feature": "Coupon", "instances": 16, "metric_value": 0.9887, "depth": 4}
				if obj[1]<=3:
					# {"feature": "Distance", "instances": 10, "metric_value": 0.8813, "depth": 5}
					if obj[6]>1:
						# {"feature": "Restaurant20to50", "instances": 7, "metric_value": 0.9852, "depth": 6}
						if obj[4]<=1.0:
							# {"feature": "Direction_same", "instances": 4, "metric_value": 1.0, "depth": 7}
							if obj[5]<=0:
								return 'True'
							else: return 'True'
						elif obj[4]>1.0:
							# {"feature": "Direction_same", "instances": 3, "metric_value": 0.9183, "depth": 7}
							if obj[5]<=0:
								return 'True'
							else: return 'True'
						else: return 'True'
					elif obj[6]<=1:
						return 'True'
					else: return 'True'
				elif obj[1]>3:
					# {"feature": "Distance", "instances": 6, "metric_value": 0.9183, "depth": 5}
					if obj[6]<=1:
						# {"feature": "Restaurant20to50", "instances": 5, "metric_value": 0.7219, "depth": 6}
						if obj[4]>0.0:
							# {"feature": "Direction_same", "instances": 4, "metric_value": 0.8113, "depth": 7}
							if obj[5]<=0:
								return 'False'
							else: return 'False'
						elif obj[4]<=0.0:
							return 'False'
						else: return 'False'
					elif obj[6]>1:
						return 'True'
					else: return 'True'
				else: return 'False'
			elif obj[2]<=0:
				# {"feature": "Coupon", "instances": 13, "metric_value": 0.6194, "depth": 4}
				if obj[1]>2:
					# {"feature": "Distance", "instances": 8, "metric_value": 0.8113, "depth": 5}
					if obj[6]<=1:
						return 'True'
					elif obj[6]>1:
						# {"feature": "Restaurant20to50", "instances": 4, "metric_value": 1.0, "depth": 6}
						if obj[4]>0.0:
							# {"feature": "Direction_same", "instances": 3, "metric_value": 0.9183, "depth": 7}
							if obj[5]<=0:
								return 'False'
							else: return 'False'
						elif obj[4]<=0.0:
							return 'True'
						else: return 'True'
					else: return 'True'
				elif obj[1]<=2:
					return 'True'
				else: return 'True'
			else: return 'True'
		elif obj[3]>12:
			return 'True'
		else: return 'True'
	else: return 'True'
