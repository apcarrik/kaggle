def findDecision(obj): #obj[0]: Passanger, obj[1]: Coupon, obj[2]: Education, obj[3]: Occupation, obj[4]: Restaurant20to50, obj[5]: Direction_same, obj[6]: Distance
	# {"feature": "Coupon", "instances": 85, "metric_value": 0.9637, "depth": 1}
	if obj[1]>1:
		# {"feature": "Passanger", "instances": 62, "metric_value": 0.8691, "depth": 2}
		if obj[0]<=2:
			# {"feature": "Occupation", "instances": 42, "metric_value": 0.9587, "depth": 3}
			if obj[3]>6:
				# {"feature": "Distance", "instances": 21, "metric_value": 0.9984, "depth": 4}
				if obj[6]<=2:
					# {"feature": "Education", "instances": 18, "metric_value": 0.9911, "depth": 5}
					if obj[2]>1:
						# {"feature": "Restaurant20to50", "instances": 10, "metric_value": 0.8813, "depth": 6}
						if obj[4]>-1.0:
							# {"feature": "Direction_same", "instances": 9, "metric_value": 0.9183, "depth": 7}
							if obj[5]<=0:
								return 'True'
							elif obj[5]>0:
								return 'True'
							else: return 'True'
						elif obj[4]<=-1.0:
							return 'True'
						else: return 'True'
					elif obj[2]<=1:
						# {"feature": "Restaurant20to50", "instances": 8, "metric_value": 0.9544, "depth": 6}
						if obj[4]<=1.0:
							# {"feature": "Direction_same", "instances": 7, "metric_value": 0.8631, "depth": 7}
							if obj[5]<=0:
								return 'False'
							elif obj[5]>0:
								return 'False'
							else: return 'False'
						elif obj[4]>1.0:
							return 'True'
						else: return 'True'
					else: return 'False'
				elif obj[6]>2:
					return 'False'
				else: return 'False'
			elif obj[3]<=6:
				# {"feature": "Education", "instances": 21, "metric_value": 0.7919, "depth": 4}
				if obj[2]<=2:
					# {"feature": "Restaurant20to50", "instances": 16, "metric_value": 0.896, "depth": 5}
					if obj[4]<=1.0:
						# {"feature": "Distance", "instances": 8, "metric_value": 1.0, "depth": 6}
						if obj[6]>1:
							# {"feature": "Direction_same", "instances": 6, "metric_value": 0.9183, "depth": 7}
							if obj[5]<=0:
								return 'False'
							else: return 'False'
						elif obj[6]<=1:
							return 'True'
						else: return 'True'
					elif obj[4]>1.0:
						# {"feature": "Direction_same", "instances": 8, "metric_value": 0.5436, "depth": 6}
						if obj[5]<=0:
							# {"feature": "Distance", "instances": 6, "metric_value": 0.65, "depth": 7}
							if obj[6]<=1:
								return 'True'
							elif obj[6]>1:
								return 'True'
							else: return 'True'
						elif obj[5]>0:
							return 'True'
						else: return 'True'
					else: return 'True'
				elif obj[2]>2:
					return 'True'
				else: return 'True'
			else: return 'True'
		elif obj[0]>2:
			# {"feature": "Occupation", "instances": 20, "metric_value": 0.469, "depth": 3}
			if obj[3]>6:
				return 'True'
			elif obj[3]<=6:
				# {"feature": "Distance", "instances": 6, "metric_value": 0.9183, "depth": 4}
				if obj[6]<=1:
					return 'True'
				elif obj[6]>1:
					# {"feature": "Education", "instances": 3, "metric_value": 0.9183, "depth": 5}
					if obj[2]>0:
						return 'False'
					elif obj[2]<=0:
						return 'True'
					else: return 'True'
				else: return 'False'
			else: return 'True'
		else: return 'True'
	elif obj[1]<=1:
		# {"feature": "Distance", "instances": 23, "metric_value": 0.9321, "depth": 2}
		if obj[6]<=2:
			# {"feature": "Restaurant20to50", "instances": 18, "metric_value": 0.9911, "depth": 3}
			if obj[4]<=1.0:
				# {"feature": "Occupation", "instances": 11, "metric_value": 0.8454, "depth": 4}
				if obj[3]>6:
					# {"feature": "Direction_same", "instances": 7, "metric_value": 0.9852, "depth": 5}
					if obj[5]<=0:
						# {"feature": "Education", "instances": 6, "metric_value": 0.9183, "depth": 6}
						if obj[2]<=2:
							# {"feature": "Passanger", "instances": 5, "metric_value": 0.971, "depth": 7}
							if obj[0]<=1:
								return 'False'
							elif obj[0]>1:
								return 'False'
							else: return 'False'
						elif obj[2]>2:
							return 'False'
						else: return 'False'
					elif obj[5]>0:
						return 'True'
					else: return 'True'
				elif obj[3]<=6:
					return 'False'
				else: return 'False'
			elif obj[4]>1.0:
				# {"feature": "Occupation", "instances": 7, "metric_value": 0.8631, "depth": 4}
				if obj[3]>4:
					return 'True'
				elif obj[3]<=4:
					# {"feature": "Passanger", "instances": 3, "metric_value": 0.9183, "depth": 5}
					if obj[0]>0:
						return 'False'
					elif obj[0]<=0:
						return 'True'
					else: return 'True'
				else: return 'False'
			else: return 'True'
		elif obj[6]>2:
			return 'False'
		else: return 'False'
	else: return 'False'
