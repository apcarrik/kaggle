def findDecision(obj): #obj[0]: Passanger, obj[1]: Time, obj[2]: Coupon, obj[3]: Education, obj[4]: Occupation, obj[5]: Bar, obj[6]: Restaurant20to50, obj[7]: Direction_same, obj[8]: Distance
	# {"feature": "Passanger", "instances": 85, "metric_value": 0.9774, "depth": 1}
	if obj[0]<=1:
		# {"feature": "Occupation", "instances": 54, "metric_value": 0.999, "depth": 2}
		if obj[4]<=15:
			# {"feature": "Coupon", "instances": 45, "metric_value": 0.9825, "depth": 3}
			if obj[2]<=3:
				# {"feature": "Distance", "instances": 30, "metric_value": 1.0, "depth": 4}
				if obj[8]<=2:
					# {"feature": "Restaurant20to50", "instances": 19, "metric_value": 0.9495, "depth": 5}
					if obj[6]<=1.0:
						# {"feature": "Bar", "instances": 10, "metric_value": 0.971, "depth": 6}
						if obj[5]<=1.0:
							# {"feature": "Education", "instances": 7, "metric_value": 0.9852, "depth": 7}
							if obj[3]<=3:
								# {"feature": "Time", "instances": 6, "metric_value": 0.9183, "depth": 8}
								if obj[1]>0:
									# {"feature": "Direction_same", "instances": 5, "metric_value": 0.971, "depth": 9}
									if obj[7]<=0:
										return 'True'
									elif obj[7]>0:
										return 'True'
									else: return 'True'
								elif obj[1]<=0:
									return 'True'
								else: return 'True'
							elif obj[3]>3:
								return 'False'
							else: return 'False'
						elif obj[5]>1.0:
							return 'False'
						else: return 'False'
					elif obj[6]>1.0:
						# {"feature": "Time", "instances": 9, "metric_value": 0.5033, "depth": 6}
						if obj[1]<=1:
							return 'True'
						elif obj[1]>1:
							# {"feature": "Direction_same", "instances": 3, "metric_value": 0.9183, "depth": 7}
							if obj[7]<=0:
								return 'True'
							elif obj[7]>0:
								return 'False'
							else: return 'False'
						else: return 'True'
					else: return 'True'
				elif obj[8]>2:
					# {"feature": "Education", "instances": 11, "metric_value": 0.8454, "depth": 5}
					if obj[3]<=3:
						# {"feature": "Bar", "instances": 10, "metric_value": 0.7219, "depth": 6}
						if obj[5]>0.0:
							# {"feature": "Restaurant20to50", "instances": 7, "metric_value": 0.8631, "depth": 7}
							if obj[6]>0.0:
								# {"feature": "Time", "instances": 6, "metric_value": 0.9183, "depth": 8}
								if obj[1]<=1:
									# {"feature": "Direction_same", "instances": 6, "metric_value": 0.9183, "depth": 9}
									if obj[7]<=0:
										return 'False'
									else: return 'False'
								else: return 'False'
							elif obj[6]<=0.0:
								return 'False'
							else: return 'False'
						elif obj[5]<=0.0:
							return 'False'
						else: return 'False'
					elif obj[3]>3:
						return 'True'
					else: return 'True'
				else: return 'False'
			elif obj[2]>3:
				# {"feature": "Education", "instances": 15, "metric_value": 0.8366, "depth": 4}
				if obj[3]<=2:
					# {"feature": "Bar", "instances": 11, "metric_value": 0.9457, "depth": 5}
					if obj[5]<=2.0:
						# {"feature": "Restaurant20to50", "instances": 8, "metric_value": 1.0, "depth": 6}
						if obj[6]<=1.0:
							# {"feature": "Distance", "instances": 7, "metric_value": 0.9852, "depth": 7}
							if obj[8]<=1:
								# {"feature": "Time", "instances": 5, "metric_value": 0.971, "depth": 8}
								if obj[1]<=1:
									# {"feature": "Direction_same", "instances": 4, "metric_value": 1.0, "depth": 9}
									if obj[7]>0:
										return 'False'
									elif obj[7]<=0:
										return 'False'
									else: return 'False'
								elif obj[1]>1:
									return 'False'
								else: return 'False'
							elif obj[8]>1:
								return 'True'
							else: return 'True'
						elif obj[6]>1.0:
							return 'False'
						else: return 'False'
					elif obj[5]>2.0:
						return 'False'
					else: return 'False'
				elif obj[3]>2:
					return 'False'
				else: return 'False'
			else: return 'False'
		elif obj[4]>15:
			# {"feature": "Education", "instances": 9, "metric_value": 0.7642, "depth": 3}
			if obj[3]<=2:
				return 'True'
			elif obj[3]>2:
				return 'False'
			else: return 'False'
		else: return 'True'
	elif obj[0]>1:
		# {"feature": "Education", "instances": 31, "metric_value": 0.7706, "depth": 2}
		if obj[3]>0:
			# {"feature": "Distance", "instances": 22, "metric_value": 0.9024, "depth": 3}
			if obj[8]>1:
				# {"feature": "Bar", "instances": 15, "metric_value": 0.7219, "depth": 4}
				if obj[5]<=1.0:
					# {"feature": "Occupation", "instances": 9, "metric_value": 0.9183, "depth": 5}
					if obj[4]>2:
						# {"feature": "Coupon", "instances": 8, "metric_value": 0.8113, "depth": 6}
						if obj[2]<=1:
							# {"feature": "Time", "instances": 4, "metric_value": 1.0, "depth": 7}
							if obj[1]>0:
								# {"feature": "Restaurant20to50", "instances": 3, "metric_value": 0.9183, "depth": 8}
								if obj[6]>0.0:
									return 'True'
								elif obj[6]<=0.0:
									return 'False'
								else: return 'False'
							elif obj[1]<=0:
								return 'False'
							else: return 'False'
						elif obj[2]>1:
							return 'True'
						else: return 'True'
					elif obj[4]<=2:
						return 'False'
					else: return 'False'
				elif obj[5]>1.0:
					return 'True'
				else: return 'True'
			elif obj[8]<=1:
				# {"feature": "Time", "instances": 7, "metric_value": 0.9852, "depth": 4}
				if obj[1]>0:
					# {"feature": "Occupation", "instances": 5, "metric_value": 0.971, "depth": 5}
					if obj[4]>2:
						# {"feature": "Coupon", "instances": 3, "metric_value": 0.9183, "depth": 6}
						if obj[2]<=2:
							# {"feature": "Bar", "instances": 2, "metric_value": 1.0, "depth": 7}
							if obj[5]>2.0:
								return 'False'
							elif obj[5]<=2.0:
								return 'True'
							else: return 'True'
						elif obj[2]>2:
							return 'False'
						else: return 'False'
					elif obj[4]<=2:
						return 'True'
					else: return 'True'
				elif obj[1]<=0:
					return 'False'
				else: return 'False'
			else: return 'False'
		elif obj[3]<=0:
			return 'True'
		else: return 'True'
	else: return 'True'
