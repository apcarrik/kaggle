def findDecision(obj): #obj[0]: Passanger, obj[1]: Time, obj[2]: Coupon, obj[3]: Education, obj[4]: Occupation, obj[5]: Bar, obj[6]: Restaurant20to50, obj[7]: Direction_same, obj[8]: Distance
	# {"feature": "Passanger", "instances": 51, "metric_value": 0.9997, "depth": 1}
	if obj[0]<=2:
		# {"feature": "Education", "instances": 41, "metric_value": 0.9892, "depth": 2}
		if obj[3]>1:
			# {"feature": "Direction_same", "instances": 31, "metric_value": 0.9383, "depth": 3}
			if obj[7]<=0:
				# {"feature": "Distance", "instances": 25, "metric_value": 0.9896, "depth": 4}
				if obj[8]<=2:
					# {"feature": "Occupation", "instances": 19, "metric_value": 0.998, "depth": 5}
					if obj[4]<=18:
						# {"feature": "Coupon", "instances": 17, "metric_value": 0.9975, "depth": 6}
						if obj[2]<=2:
							# {"feature": "Time", "instances": 10, "metric_value": 0.8813, "depth": 7}
							if obj[1]<=1:
								# {"feature": "Bar", "instances": 6, "metric_value": 1.0, "depth": 8}
								if obj[5]>0.0:
									# {"feature": "Restaurant20to50", "instances": 4, "metric_value": 0.8113, "depth": 9}
									if obj[6]<=1.0:
										return 'True'
									elif obj[6]>1.0:
										return 'True'
									else: return 'True'
								elif obj[5]<=0.0:
									return 'False'
								else: return 'False'
							elif obj[1]>1:
								return 'True'
							else: return 'True'
						elif obj[2]>2:
							# {"feature": "Restaurant20to50", "instances": 7, "metric_value": 0.5917, "depth": 7}
							if obj[6]>-1.0:
								return 'False'
							elif obj[6]<=-1.0:
								return 'True'
							else: return 'True'
						else: return 'False'
					elif obj[4]>18:
						return 'True'
					else: return 'True'
				elif obj[8]>2:
					# {"feature": "Coupon", "instances": 6, "metric_value": 0.65, "depth": 5}
					if obj[2]>0:
						# {"feature": "Time", "instances": 3, "metric_value": 0.9183, "depth": 6}
						if obj[1]<=1:
							# {"feature": "Occupation", "instances": 2, "metric_value": 1.0, "depth": 7}
							if obj[4]<=5:
								return 'True'
							elif obj[4]>5:
								return 'False'
							else: return 'False'
						elif obj[1]>1:
							return 'False'
						else: return 'False'
					elif obj[2]<=0:
						return 'False'
					else: return 'False'
				else: return 'False'
			elif obj[7]>0:
				return 'False'
			else: return 'False'
		elif obj[3]<=1:
			# {"feature": "Restaurant20to50", "instances": 10, "metric_value": 0.8813, "depth": 3}
			if obj[6]<=2.0:
				# {"feature": "Time", "instances": 9, "metric_value": 0.7642, "depth": 4}
				if obj[1]>0:
					# {"feature": "Coupon", "instances": 6, "metric_value": 0.9183, "depth": 5}
					if obj[2]<=3:
						# {"feature": "Occupation", "instances": 5, "metric_value": 0.7219, "depth": 6}
						if obj[4]<=7:
							return 'True'
						elif obj[4]>7:
							# {"feature": "Distance", "instances": 2, "metric_value": 1.0, "depth": 7}
							if obj[8]>1:
								return 'True'
							elif obj[8]<=1:
								return 'False'
							else: return 'False'
						else: return 'True'
					elif obj[2]>3:
						return 'False'
					else: return 'False'
				elif obj[1]<=0:
					return 'True'
				else: return 'True'
			elif obj[6]>2.0:
				return 'False'
			else: return 'False'
		else: return 'True'
	elif obj[0]>2:
		# {"feature": "Education", "instances": 10, "metric_value": 0.7219, "depth": 2}
		if obj[3]>0:
			return 'True'
		elif obj[3]<=0:
			# {"feature": "Bar", "instances": 4, "metric_value": 1.0, "depth": 3}
			if obj[5]<=0.0:
				return 'False'
			elif obj[5]>0.0:
				return 'True'
			else: return 'True'
		else: return 'False'
	else: return 'True'
