def findDecision(obj): #obj[0]: Passanger, obj[1]: Time, obj[2]: Coupon, obj[3]: Education, obj[4]: Occupation, obj[5]: Bar, obj[6]: Restaurant20to50, obj[7]: Direction_same, obj[8]: Distance
	# {"feature": "Occupation", "instances": 51, "metric_value": 0.9975, "depth": 1}
	if obj[4]>1:
		# {"feature": "Bar", "instances": 43, "metric_value": 0.9965, "depth": 2}
		if obj[5]<=2.0:
			# {"feature": "Coupon", "instances": 32, "metric_value": 0.9745, "depth": 3}
			if obj[2]>1:
				# {"feature": "Passanger", "instances": 22, "metric_value": 0.994, "depth": 4}
				if obj[0]>0:
					# {"feature": "Education", "instances": 20, "metric_value": 1.0, "depth": 5}
					if obj[3]>1:
						# {"feature": "Distance", "instances": 11, "metric_value": 0.9457, "depth": 6}
						if obj[8]<=2:
							# {"feature": "Direction_same", "instances": 10, "metric_value": 0.8813, "depth": 7}
							if obj[7]<=0:
								# {"feature": "Time", "instances": 7, "metric_value": 0.5917, "depth": 8}
								if obj[1]>0:
									# {"feature": "Restaurant20to50", "instances": 4, "metric_value": 0.8113, "depth": 9}
									if obj[6]>0.0:
										return 'False'
									elif obj[6]<=0.0:
										return 'False'
									else: return 'False'
								elif obj[1]<=0:
									return 'False'
								else: return 'False'
							elif obj[7]>0:
								# {"feature": "Time", "instances": 3, "metric_value": 0.9183, "depth": 8}
								if obj[1]<=0:
									return 'True'
								elif obj[1]>0:
									return 'False'
								else: return 'False'
							else: return 'True'
						elif obj[8]>2:
							return 'True'
						else: return 'True'
					elif obj[3]<=1:
						# {"feature": "Time", "instances": 9, "metric_value": 0.9183, "depth": 6}
						if obj[1]<=1:
							# {"feature": "Restaurant20to50", "instances": 5, "metric_value": 0.971, "depth": 7}
							if obj[6]>0.0:
								# {"feature": "Direction_same", "instances": 4, "metric_value": 1.0, "depth": 8}
								if obj[7]>0:
									# {"feature": "Distance", "instances": 2, "metric_value": 1.0, "depth": 9}
									if obj[8]<=1:
										return 'True'
									else: return 'True'
								elif obj[7]<=0:
									# {"feature": "Distance", "instances": 2, "metric_value": 1.0, "depth": 9}
									if obj[8]<=1:
										return 'True'
									else: return 'True'
								else: return 'True'
							elif obj[6]<=0.0:
								return 'False'
							else: return 'False'
						elif obj[1]>1:
							return 'True'
						else: return 'True'
					else: return 'True'
				elif obj[0]<=0:
					return 'True'
				else: return 'True'
			elif obj[2]<=1:
				# {"feature": "Education", "instances": 10, "metric_value": 0.469, "depth": 4}
				if obj[3]>0:
					return 'False'
				elif obj[3]<=0:
					# {"feature": "Passanger", "instances": 2, "metric_value": 1.0, "depth": 5}
					if obj[0]<=1:
						return 'True'
					elif obj[0]>1:
						return 'False'
					else: return 'False'
				else: return 'True'
			else: return 'False'
		elif obj[5]>2.0:
			# {"feature": "Distance", "instances": 11, "metric_value": 0.9457, "depth": 3}
			if obj[8]<=2:
				# {"feature": "Time", "instances": 9, "metric_value": 0.7642, "depth": 4}
				if obj[1]<=3:
					return 'True'
				elif obj[1]>3:
					# {"feature": "Coupon", "instances": 3, "metric_value": 0.9183, "depth": 5}
					if obj[2]>0:
						return 'False'
					elif obj[2]<=0:
						return 'True'
					else: return 'True'
				else: return 'False'
			elif obj[8]>2:
				return 'False'
			else: return 'False'
		else: return 'True'
	elif obj[4]<=1:
		# {"feature": "Time", "instances": 8, "metric_value": 0.5436, "depth": 2}
		if obj[1]>0:
			return 'True'
		elif obj[1]<=0:
			return 'False'
		else: return 'False'
	else: return 'True'
