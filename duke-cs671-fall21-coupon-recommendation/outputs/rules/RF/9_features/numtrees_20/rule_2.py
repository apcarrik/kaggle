def findDecision(obj): #obj[0]: Passanger, obj[1]: Time, obj[2]: Coupon, obj[3]: Education, obj[4]: Occupation, obj[5]: Bar, obj[6]: Restaurant20to50, obj[7]: Direction_same, obj[8]: Distance
	# {"feature": "Coupon", "instances": 51, "metric_value": 0.9931, "depth": 1}
	if obj[2]>0:
		# {"feature": "Distance", "instances": 47, "metric_value": 0.9734, "depth": 2}
		if obj[8]>1:
			# {"feature": "Occupation", "instances": 34, "metric_value": 1.0, "depth": 3}
			if obj[4]>3:
				# {"feature": "Education", "instances": 29, "metric_value": 0.9923, "depth": 4}
				if obj[3]<=2:
					# {"feature": "Passanger", "instances": 22, "metric_value": 0.9457, "depth": 5}
					if obj[0]>0:
						# {"feature": "Bar", "instances": 21, "metric_value": 0.9183, "depth": 6}
						if obj[5]<=1.0:
							# {"feature": "Time", "instances": 14, "metric_value": 0.9852, "depth": 7}
							if obj[1]>0:
								# {"feature": "Restaurant20to50", "instances": 8, "metric_value": 0.9544, "depth": 8}
								if obj[6]<=1.0:
									# {"feature": "Direction_same", "instances": 7, "metric_value": 0.8631, "depth": 9}
									if obj[7]<=0:
										return 'False'
									else: return 'False'
								elif obj[6]>1.0:
									return 'True'
								else: return 'True'
							elif obj[1]<=0:
								# {"feature": "Restaurant20to50", "instances": 6, "metric_value": 0.65, "depth": 8}
								if obj[6]>0.0:
									return 'True'
								elif obj[6]<=0.0:
									# {"feature": "Direction_same", "instances": 2, "metric_value": 1.0, "depth": 9}
									if obj[7]<=0:
										return 'True'
									else: return 'True'
								else: return 'True'
							else: return 'True'
						elif obj[5]>1.0:
							# {"feature": "Time", "instances": 7, "metric_value": 0.5917, "depth": 7}
							if obj[1]>0:
								return 'True'
							elif obj[1]<=0:
								# {"feature": "Restaurant20to50", "instances": 2, "metric_value": 1.0, "depth": 8}
								if obj[6]<=1.0:
									return 'True'
								elif obj[6]>1.0:
									return 'False'
								else: return 'False'
							else: return 'True'
						else: return 'True'
					elif obj[0]<=0:
						return 'False'
					else: return 'False'
				elif obj[3]>2:
					# {"feature": "Passanger", "instances": 7, "metric_value": 0.8631, "depth": 5}
					if obj[0]>2:
						# {"feature": "Time", "instances": 4, "metric_value": 1.0, "depth": 6}
						if obj[1]>0:
							# {"feature": "Restaurant20to50", "instances": 3, "metric_value": 0.9183, "depth": 7}
							if obj[6]<=1.0:
								return 'True'
							elif obj[6]>1.0:
								return 'False'
							else: return 'False'
						elif obj[1]<=0:
							return 'False'
						else: return 'False'
					elif obj[0]<=2:
						return 'False'
					else: return 'False'
				else: return 'False'
			elif obj[4]<=3:
				# {"feature": "Bar", "instances": 5, "metric_value": 0.7219, "depth": 4}
				if obj[5]<=1.0:
					return 'False'
				elif obj[5]>1.0:
					# {"feature": "Restaurant20to50", "instances": 2, "metric_value": 1.0, "depth": 5}
					if obj[6]>1.0:
						return 'True'
					elif obj[6]<=1.0:
						return 'False'
					else: return 'False'
				else: return 'True'
			else: return 'False'
		elif obj[8]<=1:
			# {"feature": "Education", "instances": 13, "metric_value": 0.6194, "depth": 3}
			if obj[3]>0:
				return 'True'
			elif obj[3]<=0:
				# {"feature": "Occupation", "instances": 6, "metric_value": 0.9183, "depth": 4}
				if obj[4]>5:
					# {"feature": "Time", "instances": 5, "metric_value": 0.7219, "depth": 5}
					if obj[1]<=0:
						# {"feature": "Passanger", "instances": 3, "metric_value": 0.9183, "depth": 6}
						if obj[0]>0:
							# {"feature": "Restaurant20to50", "instances": 2, "metric_value": 1.0, "depth": 7}
							if obj[6]>2.0:
								return 'True'
							elif obj[6]<=2.0:
								return 'False'
							else: return 'False'
						elif obj[0]<=0:
							return 'True'
						else: return 'True'
					elif obj[1]>0:
						return 'True'
					else: return 'True'
				elif obj[4]<=5:
					return 'False'
				else: return 'False'
			else: return 'True'
		else: return 'True'
	elif obj[2]<=0:
		return 'False'
	else: return 'False'
