def findDecision(obj): #obj[0]: Passanger, obj[1]: Time, obj[2]: Coupon, obj[3]: Gender, obj[4]: Age, obj[5]: Education, obj[6]: Occupation, obj[7]: Bar, obj[8]: Coffeehouse, obj[9]: Restaurant20to50, obj[10]: Direction_same, obj[11]: Distance
	# {"feature": "Coupon", "instances": 34, "metric_value": 0.9774, "depth": 1}
	if obj[2]>0:
		# {"feature": "Occupation", "instances": 31, "metric_value": 0.9383, "depth": 2}
		if obj[6]<=14:
			# {"feature": "Age", "instances": 25, "metric_value": 0.795, "depth": 3}
			if obj[4]<=4:
				# {"feature": "Coffeehouse", "instances": 20, "metric_value": 0.6098, "depth": 4}
				if obj[8]>0.0:
					return 'True'
				elif obj[8]<=0.0:
					# {"feature": "Gender", "instances": 10, "metric_value": 0.8813, "depth": 5}
					if obj[3]>0:
						# {"feature": "Distance", "instances": 7, "metric_value": 0.9852, "depth": 6}
						if obj[11]<=1:
							# {"feature": "Bar", "instances": 5, "metric_value": 0.7219, "depth": 7}
							if obj[7]<=0.0:
								return 'True'
							elif obj[7]>0.0:
								return 'False'
							else: return 'False'
						elif obj[11]>1:
							return 'False'
						else: return 'False'
					elif obj[3]<=0:
						return 'True'
					else: return 'True'
				else: return 'True'
			elif obj[4]>4:
				# {"feature": "Education", "instances": 5, "metric_value": 0.971, "depth": 4}
				if obj[5]<=1:
					# {"feature": "Restaurant20to50", "instances": 3, "metric_value": 0.9183, "depth": 5}
					if obj[9]>1.0:
						return 'True'
					elif obj[9]<=1.0:
						return 'False'
					else: return 'False'
				elif obj[5]>1:
					return 'False'
				else: return 'False'
			else: return 'False'
		elif obj[6]>14:
			# {"feature": "Age", "instances": 6, "metric_value": 0.65, "depth": 3}
			if obj[4]<=2:
				return 'False'
			elif obj[4]>2:
				return 'True'
			else: return 'True'
		else: return 'False'
	elif obj[2]<=0:
		return 'False'
	else: return 'False'
