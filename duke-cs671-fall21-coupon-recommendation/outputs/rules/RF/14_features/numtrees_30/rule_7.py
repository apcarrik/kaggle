def findDecision(obj): #obj[0]: Passanger, obj[1]: Time, obj[2]: Coupon, obj[3]: Gender, obj[4]: Age, obj[5]: Children, obj[6]: Education, obj[7]: Occupation, obj[8]: Income, obj[9]: Bar, obj[10]: Coffeehouse, obj[11]: Restaurant20to50, obj[12]: Direction_same, obj[13]: Distance
	# {"feature": "Bar", "instances": 34, "metric_value": 0.9082, "depth": 1}
	if obj[9]<=2.0:
		# {"feature": "Income", "instances": 30, "metric_value": 0.8366, "depth": 2}
		if obj[8]<=4:
			# {"feature": "Occupation", "instances": 18, "metric_value": 0.9641, "depth": 3}
			if obj[7]<=14:
				# {"feature": "Coupon", "instances": 15, "metric_value": 0.9968, "depth": 4}
				if obj[2]>1:
					# {"feature": "Time", "instances": 13, "metric_value": 0.9612, "depth": 5}
					if obj[1]<=2:
						# {"feature": "Coffeehouse", "instances": 10, "metric_value": 1.0, "depth": 6}
						if obj[10]<=2.0:
							# {"feature": "Age", "instances": 8, "metric_value": 0.9544, "depth": 7}
							if obj[4]>2:
								# {"feature": "Direction_same", "instances": 5, "metric_value": 0.971, "depth": 8}
								if obj[12]<=0:
									return 'False'
								elif obj[12]>0:
									return 'True'
								else: return 'True'
							elif obj[4]<=2:
								return 'True'
							else: return 'True'
						elif obj[10]>2.0:
							return 'False'
						else: return 'False'
					elif obj[1]>2:
						return 'False'
					else: return 'False'
				elif obj[2]<=1:
					return 'True'
				else: return 'True'
			elif obj[7]>14:
				return 'False'
			else: return 'False'
		elif obj[8]>4:
			# {"feature": "Coffeehouse", "instances": 12, "metric_value": 0.4138, "depth": 3}
			if obj[10]<=1.0:
				return 'False'
			elif obj[10]>1.0:
				# {"feature": "Age", "instances": 3, "metric_value": 0.9183, "depth": 4}
				if obj[4]>1:
					return 'False'
				elif obj[4]<=1:
					return 'True'
				else: return 'True'
			else: return 'False'
		else: return 'False'
	elif obj[9]>2.0:
		# {"feature": "Passanger", "instances": 4, "metric_value": 0.8113, "depth": 2}
		if obj[0]>1:
			# {"feature": "Time", "instances": 2, "metric_value": 1.0, "depth": 3}
			if obj[1]<=0:
				return 'False'
			elif obj[1]>0:
				return 'True'
			else: return 'True'
		elif obj[0]<=1:
			return 'True'
		else: return 'True'
	else: return 'True'
