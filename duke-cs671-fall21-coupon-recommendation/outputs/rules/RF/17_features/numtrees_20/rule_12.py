def findDecision(obj): #obj[0]: Passanger, obj[1]: Weather, obj[2]: Time, obj[3]: Coupon, obj[4]: Coupon_validity, obj[5]: Gender, obj[6]: Age, obj[7]: Maritalstatus, obj[8]: Children, obj[9]: Education, obj[10]: Occupation, obj[11]: Income, obj[12]: Bar, obj[13]: Coffeehouse, obj[14]: Restaurant20to50, obj[15]: Direction_same, obj[16]: Distance
	# {"feature": "Coffeehouse", "instances": 51, "metric_value": 0.9864, "depth": 1}
	if obj[13]>1.0:
		# {"feature": "Time", "instances": 27, "metric_value": 0.8256, "depth": 2}
		if obj[2]>0:
			# {"feature": "Distance", "instances": 17, "metric_value": 0.9774, "depth": 3}
			if obj[16]<=2:
				# {"feature": "Weather", "instances": 15, "metric_value": 0.9183, "depth": 4}
				if obj[1]<=0:
					# {"feature": "Occupation", "instances": 10, "metric_value": 0.7219, "depth": 5}
					if obj[10]<=12:
						return 'True'
					elif obj[10]>12:
						# {"feature": "Passanger", "instances": 4, "metric_value": 1.0, "depth": 6}
						if obj[0]<=1:
							# {"feature": "Education", "instances": 3, "metric_value": 0.9183, "depth": 7}
							if obj[9]<=3:
								return 'False'
							elif obj[9]>3:
								return 'True'
							else: return 'True'
						elif obj[0]>1:
							return 'True'
						else: return 'True'
					else: return 'True'
				elif obj[1]>0:
					# {"feature": "Children", "instances": 5, "metric_value": 0.971, "depth": 5}
					if obj[8]>0:
						return 'False'
					elif obj[8]<=0:
						return 'True'
					else: return 'True'
				else: return 'False'
			elif obj[16]>2:
				return 'False'
			else: return 'False'
		elif obj[2]<=0:
			return 'True'
		else: return 'True'
	elif obj[13]<=1.0:
		# {"feature": "Income", "instances": 24, "metric_value": 0.9544, "depth": 2}
		if obj[11]<=6:
			# {"feature": "Coupon", "instances": 21, "metric_value": 0.8631, "depth": 3}
			if obj[3]>1:
				# {"feature": "Coupon_validity", "instances": 13, "metric_value": 0.9957, "depth": 4}
				if obj[4]<=0:
					# {"feature": "Age", "instances": 8, "metric_value": 0.8113, "depth": 5}
					if obj[6]>2:
						return 'True'
					elif obj[6]<=2:
						# {"feature": "Maritalstatus", "instances": 4, "metric_value": 1.0, "depth": 6}
						if obj[7]<=0:
							return 'True'
						elif obj[7]>0:
							return 'False'
						else: return 'False'
					else: return 'True'
				elif obj[4]>0:
					return 'False'
				else: return 'False'
			elif obj[3]<=1:
				return 'False'
			else: return 'False'
		elif obj[11]>6:
			return 'True'
		else: return 'True'
	else: return 'False'
