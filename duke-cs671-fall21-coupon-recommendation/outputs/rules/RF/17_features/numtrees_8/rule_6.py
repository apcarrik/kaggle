def findDecision(obj): #obj[0]: Passanger, obj[1]: Weather, obj[2]: Time, obj[3]: Coupon, obj[4]: Coupon_validity, obj[5]: Gender, obj[6]: Age, obj[7]: Maritalstatus, obj[8]: Children, obj[9]: Education, obj[10]: Occupation, obj[11]: Income, obj[12]: Bar, obj[13]: Coffeehouse, obj[14]: Restaurant20to50, obj[15]: Direction_same, obj[16]: Distance
	# {"feature": "Time", "instances": 127, "metric_value": 0.9899, "depth": 1}
	if obj[2]<=3:
		# {"feature": "Coupon", "instances": 103, "metric_value": 0.9999, "depth": 2}
		if obj[3]>1:
			# {"feature": "Bar", "instances": 69, "metric_value": 0.9742, "depth": 3}
			if obj[12]>0.0:
				# {"feature": "Passanger", "instances": 44, "metric_value": 0.994, "depth": 4}
				if obj[0]<=2:
					# {"feature": "Education", "instances": 29, "metric_value": 0.8936, "depth": 5}
					if obj[9]<=2:
						# {"feature": "Age", "instances": 22, "metric_value": 0.976, "depth": 6}
						if obj[6]>0:
							# {"feature": "Coffeehouse", "instances": 20, "metric_value": 0.9341, "depth": 7}
							if obj[13]<=2.0:
								# {"feature": "Weather", "instances": 18, "metric_value": 0.8524, "depth": 8}
								if obj[1]<=0:
									# {"feature": "Restaurant20to50", "instances": 13, "metric_value": 0.9612, "depth": 9}
									if obj[14]>0.0:
										# {"feature": "Maritalstatus", "instances": 10, "metric_value": 1.0, "depth": 10}
										if obj[7]<=0:
											# {"feature": "Income", "instances": 7, "metric_value": 0.8631, "depth": 11}
											if obj[11]<=6:
												# {"feature": "Occupation", "instances": 6, "metric_value": 0.65, "depth": 12}
												if obj[10]>4:
													# {"feature": "Direction_same", "instances": 3, "metric_value": 0.9183, "depth": 13}
													if obj[15]<=0:
														return 'False'
													elif obj[15]>0:
														return 'True'
													else: return 'True'
												elif obj[10]<=4:
													return 'False'
												else: return 'False'
											elif obj[11]>6:
												return 'True'
											else: return 'True'
										elif obj[7]>0:
											return 'True'
										else: return 'True'
									elif obj[14]<=0.0:
										return 'False'
									else: return 'False'
								elif obj[1]>0:
									return 'False'
								else: return 'False'
							elif obj[13]>2.0:
								return 'True'
							else: return 'True'
						elif obj[6]<=0:
							return 'True'
						else: return 'True'
					elif obj[9]>2:
						return 'False'
					else: return 'False'
				elif obj[0]>2:
					# {"feature": "Age", "instances": 15, "metric_value": 0.8366, "depth": 5}
					if obj[6]<=3:
						# {"feature": "Education", "instances": 11, "metric_value": 0.9457, "depth": 6}
						if obj[9]<=2:
							# {"feature": "Maritalstatus", "instances": 6, "metric_value": 0.65, "depth": 7}
							if obj[7]>0:
								return 'True'
							elif obj[7]<=0:
								# {"feature": "Coupon_validity", "instances": 2, "metric_value": 1.0, "depth": 8}
								if obj[4]>0:
									return 'False'
								elif obj[4]<=0:
									return 'True'
								else: return 'True'
							else: return 'False'
						elif obj[9]>2:
							# {"feature": "Income", "instances": 5, "metric_value": 0.971, "depth": 7}
							if obj[11]>2:
								# {"feature": "Children", "instances": 3, "metric_value": 0.9183, "depth": 8}
								if obj[8]<=0:
									return 'True'
								elif obj[8]>0:
									return 'False'
								else: return 'False'
							elif obj[11]<=2:
								return 'False'
							else: return 'False'
						else: return 'False'
					elif obj[6]>3:
						return 'True'
					else: return 'True'
				else: return 'True'
			elif obj[12]<=0.0:
				# {"feature": "Occupation", "instances": 25, "metric_value": 0.6343, "depth": 4}
				if obj[10]>5:
					# {"feature": "Direction_same", "instances": 15, "metric_value": 0.8366, "depth": 5}
					if obj[15]<=0:
						# {"feature": "Maritalstatus", "instances": 11, "metric_value": 0.9457, "depth": 6}
						if obj[7]<=1:
							# {"feature": "Education", "instances": 9, "metric_value": 0.7642, "depth": 7}
							if obj[9]<=0:
								return 'True'
							elif obj[9]>0:
								# {"feature": "Distance", "instances": 4, "metric_value": 1.0, "depth": 8}
								if obj[16]>1:
									return 'True'
								elif obj[16]<=1:
									return 'False'
								else: return 'False'
							else: return 'True'
						elif obj[7]>1:
							return 'False'
						else: return 'False'
					elif obj[15]>0:
						return 'True'
					else: return 'True'
				elif obj[10]<=5:
					return 'True'
				else: return 'True'
			else: return 'True'
		elif obj[3]<=1:
			# {"feature": "Bar", "instances": 34, "metric_value": 0.874, "depth": 3}
			if obj[12]<=2.0:
				# {"feature": "Distance", "instances": 29, "metric_value": 0.6632, "depth": 4}
				if obj[16]>1:
					return 'False'
				elif obj[16]<=1:
					# {"feature": "Coffeehouse", "instances": 13, "metric_value": 0.9612, "depth": 5}
					if obj[13]<=2.0:
						# {"feature": "Restaurant20to50", "instances": 10, "metric_value": 0.7219, "depth": 6}
						if obj[14]<=2.0:
							return 'False'
						elif obj[14]>2.0:
							return 'True'
						else: return 'True'
					elif obj[13]>2.0:
						return 'True'
					else: return 'True'
				else: return 'False'
			elif obj[12]>2.0:
				return 'True'
			else: return 'True'
		else: return 'False'
	elif obj[2]>3:
		# {"feature": "Gender", "instances": 24, "metric_value": 0.65, "depth": 2}
		if obj[5]<=0:
			return 'True'
		elif obj[5]>0:
			# {"feature": "Coffeehouse", "instances": 12, "metric_value": 0.9183, "depth": 3}
			if obj[13]<=2.0:
				# {"feature": "Bar", "instances": 7, "metric_value": 0.5917, "depth": 4}
				if obj[12]<=1.0:
					return 'True'
				elif obj[12]>1.0:
					return 'False'
				else: return 'False'
			elif obj[13]>2.0:
				# {"feature": "Restaurant20to50", "instances": 5, "metric_value": 0.971, "depth": 4}
				if obj[14]<=1.0:
					return 'False'
				elif obj[14]>1.0:
					return 'True'
				else: return 'True'
			else: return 'False'
		else: return 'True'
	else: return 'True'
