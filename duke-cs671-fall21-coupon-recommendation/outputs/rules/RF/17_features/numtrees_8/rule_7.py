def findDecision(obj): #obj[0]: Passanger, obj[1]: Weather, obj[2]: Time, obj[3]: Coupon, obj[4]: Coupon_validity, obj[5]: Gender, obj[6]: Age, obj[7]: Maritalstatus, obj[8]: Children, obj[9]: Education, obj[10]: Occupation, obj[11]: Income, obj[12]: Bar, obj[13]: Coffeehouse, obj[14]: Restaurant20to50, obj[15]: Direction_same, obj[16]: Distance
	# {"feature": "Coupon", "instances": 127, "metric_value": 0.9946, "depth": 1}
	if obj[3]<=3:
		# {"feature": "Passanger", "instances": 84, "metric_value": 0.9403, "depth": 2}
		if obj[0]<=1:
			# {"feature": "Income", "instances": 48, "metric_value": 0.9987, "depth": 3}
			if obj[11]>2:
				# {"feature": "Occupation", "instances": 35, "metric_value": 0.9275, "depth": 4}
				if obj[10]>3:
					# {"feature": "Age", "instances": 25, "metric_value": 0.9988, "depth": 5}
					if obj[6]>1:
						# {"feature": "Coffeehouse", "instances": 19, "metric_value": 0.9495, "depth": 6}
						if obj[13]<=3.0:
							# {"feature": "Maritalstatus", "instances": 17, "metric_value": 0.874, "depth": 7}
							if obj[7]>0:
								# {"feature": "Weather", "instances": 12, "metric_value": 0.9799, "depth": 8}
								if obj[1]<=1:
									# {"feature": "Education", "instances": 10, "metric_value": 1.0, "depth": 9}
									if obj[9]>0:
										# {"feature": "Time", "instances": 6, "metric_value": 0.9183, "depth": 10}
										if obj[2]<=2:
											# {"feature": "Bar", "instances": 5, "metric_value": 0.7219, "depth": 11}
											if obj[12]>0.0:
												return 'True'
											elif obj[12]<=0.0:
												# {"feature": "Coupon_validity", "instances": 2, "metric_value": 1.0, "depth": 12}
												if obj[4]>0:
													return 'False'
												elif obj[4]<=0:
													return 'True'
												else: return 'True'
											else: return 'False'
										elif obj[2]>2:
											return 'False'
										else: return 'False'
									elif obj[9]<=0:
										# {"feature": "Time", "instances": 4, "metric_value": 0.8113, "depth": 10}
										if obj[2]<=1:
											return 'False'
										elif obj[2]>1:
											return 'True'
										else: return 'True'
									else: return 'False'
								elif obj[1]>1:
									return 'True'
								else: return 'True'
							elif obj[7]<=0:
								return 'True'
							else: return 'True'
						elif obj[13]>3.0:
							return 'False'
						else: return 'False'
					elif obj[6]<=1:
						# {"feature": "Education", "instances": 6, "metric_value": 0.65, "depth": 6}
						if obj[9]<=3:
							return 'False'
						elif obj[9]>3:
							return 'True'
						else: return 'True'
					else: return 'False'
				elif obj[10]<=3:
					return 'True'
				else: return 'True'
			elif obj[11]<=2:
				# {"feature": "Time", "instances": 13, "metric_value": 0.6194, "depth": 4}
				if obj[2]>0:
					return 'False'
				elif obj[2]<=0:
					# {"feature": "Age", "instances": 4, "metric_value": 1.0, "depth": 5}
					if obj[6]>0:
						# {"feature": "Occupation", "instances": 3, "metric_value": 0.9183, "depth": 6}
						if obj[10]<=6:
							return 'False'
						elif obj[10]>6:
							return 'True'
						else: return 'True'
					elif obj[6]<=0:
						return 'True'
					else: return 'True'
				else: return 'False'
			else: return 'False'
		elif obj[0]>1:
			# {"feature": "Time", "instances": 36, "metric_value": 0.7107, "depth": 3}
			if obj[2]>2:
				# {"feature": "Occupation", "instances": 21, "metric_value": 0.9183, "depth": 4}
				if obj[10]<=14:
					# {"feature": "Age", "instances": 18, "metric_value": 0.7642, "depth": 5}
					if obj[6]>0:
						# {"feature": "Income", "instances": 12, "metric_value": 0.4138, "depth": 6}
						if obj[11]>0:
							return 'True'
						elif obj[11]<=0:
							return 'False'
						else: return 'False'
					elif obj[6]<=0:
						# {"feature": "Maritalstatus", "instances": 6, "metric_value": 1.0, "depth": 6}
						if obj[7]>0:
							# {"feature": "Coffeehouse", "instances": 4, "metric_value": 0.8113, "depth": 7}
							if obj[13]<=1.0:
								return 'False'
							elif obj[13]>1.0:
								return 'True'
							else: return 'True'
						elif obj[7]<=0:
							return 'True'
						else: return 'True'
					else: return 'True'
				elif obj[10]>14:
					return 'False'
				else: return 'False'
			elif obj[2]<=2:
				return 'True'
			else: return 'True'
		else: return 'True'
	elif obj[3]>3:
		# {"feature": "Education", "instances": 43, "metric_value": 0.933, "depth": 2}
		if obj[9]>0:
			# {"feature": "Gender", "instances": 25, "metric_value": 0.7219, "depth": 3}
			if obj[5]>0:
				# {"feature": "Maritalstatus", "instances": 17, "metric_value": 0.874, "depth": 4}
				if obj[7]>0:
					# {"feature": "Income", "instances": 9, "metric_value": 0.5033, "depth": 5}
					if obj[11]<=4:
						return 'False'
					elif obj[11]>4:
						return 'True'
					else: return 'True'
				elif obj[7]<=0:
					# {"feature": "Income", "instances": 8, "metric_value": 1.0, "depth": 5}
					if obj[11]<=4:
						# {"feature": "Weather", "instances": 5, "metric_value": 0.7219, "depth": 6}
						if obj[1]<=0:
							return 'True'
						elif obj[1]>0:
							# {"feature": "Time", "instances": 2, "metric_value": 1.0, "depth": 7}
							if obj[2]>0:
								return 'True'
							elif obj[2]<=0:
								return 'False'
							else: return 'False'
						else: return 'True'
					elif obj[11]>4:
						return 'False'
					else: return 'False'
				else: return 'False'
			elif obj[5]<=0:
				return 'False'
			else: return 'False'
		elif obj[9]<=0:
			# {"feature": "Occupation", "instances": 18, "metric_value": 0.9911, "depth": 3}
			if obj[10]<=7:
				# {"feature": "Time", "instances": 12, "metric_value": 0.8113, "depth": 4}
				if obj[2]>1:
					# {"feature": "Income", "instances": 7, "metric_value": 0.9852, "depth": 5}
					if obj[11]>3:
						# {"feature": "Passanger", "instances": 4, "metric_value": 0.8113, "depth": 6}
						if obj[0]<=1:
							return 'False'
						elif obj[0]>1:
							return 'True'
						else: return 'True'
					elif obj[11]<=3:
						return 'True'
					else: return 'True'
				elif obj[2]<=1:
					return 'True'
				else: return 'True'
			elif obj[10]>7:
				# {"feature": "Weather", "instances": 6, "metric_value": 0.65, "depth": 4}
				if obj[1]<=1:
					return 'False'
				elif obj[1]>1:
					return 'True'
				else: return 'True'
			else: return 'False'
		else: return 'True'
	else: return 'False'
