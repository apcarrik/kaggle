def findDecision(obj): #obj[0]: Passanger, obj[1]: Coupon, obj[2]: Education, obj[3]: Occupation, obj[4]: Restaurant20to50, obj[5]: Distance
	# {"feature": "Coupon", "instances": 85, "metric_value": 0.9975, "depth": 1}
	if obj[1]<=3:
		# {"feature": "Occupation", "instances": 52, "metric_value": 0.9471, "depth": 2}
		if obj[3]<=11:
			# {"feature": "Restaurant20to50", "instances": 42, "metric_value": 0.8926, "depth": 3}
			if obj[4]>0.0:
				# {"feature": "Distance", "instances": 34, "metric_value": 0.7871, "depth": 4}
				if obj[5]>1:
					# {"feature": "Passanger", "instances": 21, "metric_value": 0.9183, "depth": 5}
					if obj[0]<=2:
						# {"feature": "Education", "instances": 14, "metric_value": 0.9852, "depth": 6}
						if obj[2]<=3:
							return 'True'
						elif obj[2]>3:
							return 'True'
						else: return 'True'
					elif obj[0]>2:
						# {"feature": "Education", "instances": 7, "metric_value": 0.5917, "depth": 6}
						if obj[2]>0:
							return 'True'
						elif obj[2]<=0:
							return 'True'
						else: return 'True'
					else: return 'True'
				elif obj[5]<=1:
					# {"feature": "Passanger", "instances": 13, "metric_value": 0.3912, "depth": 5}
					if obj[0]<=1:
						# {"feature": "Education", "instances": 8, "metric_value": 0.5436, "depth": 6}
						if obj[2]<=0:
							return 'True'
						elif obj[2]>0:
							return 'True'
						else: return 'True'
					elif obj[0]>1:
						return 'True'
					else: return 'True'
				else: return 'True'
			elif obj[4]<=0.0:
				# {"feature": "Education", "instances": 8, "metric_value": 0.9544, "depth": 4}
				if obj[2]<=0:
					# {"feature": "Distance", "instances": 5, "metric_value": 0.7219, "depth": 5}
					if obj[5]>1:
						# {"feature": "Passanger", "instances": 3, "metric_value": 0.9183, "depth": 6}
						if obj[0]<=1:
							return 'False'
						elif obj[0]>1:
							return 'False'
						else: return 'False'
					elif obj[5]<=1:
						return 'False'
					else: return 'False'
				elif obj[2]>0:
					# {"feature": "Passanger", "instances": 3, "metric_value": 0.9183, "depth": 5}
					if obj[0]<=1:
						# {"feature": "Distance", "instances": 2, "metric_value": 1.0, "depth": 6}
						if obj[5]<=1:
							return 'True'
						else: return 'True'
					elif obj[0]>1:
						return 'True'
					else: return 'True'
				else: return 'True'
			else: return 'False'
		elif obj[3]>11:
			# {"feature": "Passanger", "instances": 10, "metric_value": 0.971, "depth": 3}
			if obj[0]<=2:
				# {"feature": "Education", "instances": 7, "metric_value": 0.9852, "depth": 4}
				if obj[2]<=2:
					# {"feature": "Distance", "instances": 6, "metric_value": 1.0, "depth": 5}
					if obj[5]>1:
						# {"feature": "Restaurant20to50", "instances": 3, "metric_value": 0.9183, "depth": 6}
						if obj[4]<=1.0:
							return 'False'
						elif obj[4]>1.0:
							return 'False'
						else: return 'False'
					elif obj[5]<=1:
						# {"feature": "Restaurant20to50", "instances": 3, "metric_value": 0.9183, "depth": 6}
						if obj[4]<=1.0:
							return 'False'
						elif obj[4]>1.0:
							return 'True'
						else: return 'True'
					else: return 'True'
				elif obj[2]>2:
					return 'True'
				else: return 'True'
			elif obj[0]>2:
				return 'False'
			else: return 'False'
		else: return 'False'
	elif obj[1]>3:
		# {"feature": "Passanger", "instances": 33, "metric_value": 0.9457, "depth": 2}
		if obj[0]>1:
			# {"feature": "Occupation", "instances": 19, "metric_value": 0.998, "depth": 3}
			if obj[3]>2:
				# {"feature": "Distance", "instances": 16, "metric_value": 0.9887, "depth": 4}
				if obj[5]<=1:
					# {"feature": "Restaurant20to50", "instances": 11, "metric_value": 0.9457, "depth": 5}
					if obj[4]<=2.0:
						# {"feature": "Education", "instances": 10, "metric_value": 0.8813, "depth": 6}
						if obj[2]>0:
							return 'True'
						elif obj[2]<=0:
							return 'True'
						else: return 'True'
					elif obj[4]>2.0:
						return 'False'
					else: return 'False'
				elif obj[5]>1:
					return 'False'
				else: return 'False'
			elif obj[3]<=2:
				return 'True'
			else: return 'True'
		elif obj[0]<=1:
			# {"feature": "Occupation", "instances": 14, "metric_value": 0.5917, "depth": 3}
			if obj[3]<=11:
				return 'False'
			elif obj[3]>11:
				# {"feature": "Education", "instances": 5, "metric_value": 0.971, "depth": 4}
				if obj[2]>0:
					return 'False'
				elif obj[2]<=0:
					return 'True'
				else: return 'True'
			else: return 'False'
		else: return 'False'
	else: return 'False'
