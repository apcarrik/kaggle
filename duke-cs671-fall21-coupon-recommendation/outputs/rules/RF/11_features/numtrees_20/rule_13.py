def findDecision(obj): #obj[0]: Passanger, obj[1]: Time, obj[2]: Coupon, obj[3]: Age, obj[4]: Education, obj[5]: Occupation, obj[6]: Bar, obj[7]: Coffeehouse, obj[8]: Restaurant20to50, obj[9]: Direction_same, obj[10]: Distance
	# {"feature": "Coffeehouse", "instances": 51, "metric_value": 0.9975, "depth": 1}
	if obj[7]<=3.0:
		# {"feature": "Coupon", "instances": 46, "metric_value": 0.9986, "depth": 2}
		if obj[2]<=3:
			# {"feature": "Occupation", "instances": 33, "metric_value": 0.9457, "depth": 3}
			if obj[5]<=19:
				# {"feature": "Education", "instances": 29, "metric_value": 0.9784, "depth": 4}
				if obj[4]>0:
					# {"feature": "Restaurant20to50", "instances": 21, "metric_value": 0.9984, "depth": 5}
					if obj[8]<=2.0:
						# {"feature": "Passanger", "instances": 19, "metric_value": 0.998, "depth": 6}
						if obj[0]>0:
							# {"feature": "Bar", "instances": 15, "metric_value": 0.971, "depth": 7}
							if obj[6]<=1.0:
								# {"feature": "Time", "instances": 10, "metric_value": 1.0, "depth": 8}
								if obj[1]<=2:
									# {"feature": "Distance", "instances": 8, "metric_value": 0.9544, "depth": 9}
									if obj[10]<=2:
										# {"feature": "Age", "instances": 7, "metric_value": 0.9852, "depth": 10}
										if obj[3]>2:
											# {"feature": "Direction_same", "instances": 4, "metric_value": 1.0, "depth": 11}
											if obj[9]>0:
												return 'False'
											elif obj[9]<=0:
												return 'True'
											else: return 'True'
										elif obj[3]<=2:
											# {"feature": "Direction_same", "instances": 3, "metric_value": 0.9183, "depth": 11}
											if obj[9]<=0:
												return 'True'
											else: return 'True'
										else: return 'True'
									elif obj[10]>2:
										return 'True'
									else: return 'True'
								elif obj[1]>2:
									return 'False'
								else: return 'False'
							elif obj[6]>1.0:
								# {"feature": "Time", "instances": 5, "metric_value": 0.7219, "depth": 8}
								if obj[1]<=3:
									# {"feature": "Age", "instances": 3, "metric_value": 0.9183, "depth": 9}
									if obj[3]>2:
										# {"feature": "Direction_same", "instances": 2, "metric_value": 1.0, "depth": 10}
										if obj[9]>0:
											return 'True'
										elif obj[9]<=0:
											return 'False'
										else: return 'False'
									elif obj[3]<=2:
										return 'True'
									else: return 'True'
								elif obj[1]>3:
									return 'True'
								else: return 'True'
							else: return 'True'
						elif obj[0]<=0:
							# {"feature": "Age", "instances": 4, "metric_value": 0.8113, "depth": 7}
							if obj[3]<=2:
								return 'False'
							elif obj[3]>2:
								# {"feature": "Time", "instances": 2, "metric_value": 1.0, "depth": 8}
								if obj[1]>3:
									return 'True'
								elif obj[1]<=3:
									return 'False'
								else: return 'False'
							else: return 'True'
						else: return 'False'
					elif obj[8]>2.0:
						return 'False'
					else: return 'False'
				elif obj[4]<=0:
					# {"feature": "Age", "instances": 8, "metric_value": 0.5436, "depth": 5}
					if obj[3]>0:
						return 'True'
					elif obj[3]<=0:
						# {"feature": "Passanger", "instances": 2, "metric_value": 1.0, "depth": 6}
						if obj[0]>1:
							return 'False'
						elif obj[0]<=1:
							return 'True'
						else: return 'True'
					else: return 'False'
				else: return 'True'
			elif obj[5]>19:
				return 'True'
			else: return 'True'
		elif obj[2]>3:
			# {"feature": "Occupation", "instances": 13, "metric_value": 0.7793, "depth": 3}
			if obj[5]<=8:
				# {"feature": "Age", "instances": 8, "metric_value": 0.9544, "depth": 4}
				if obj[3]>1:
					# {"feature": "Passanger", "instances": 6, "metric_value": 0.65, "depth": 5}
					if obj[0]<=1:
						return 'False'
					elif obj[0]>1:
						# {"feature": "Time", "instances": 2, "metric_value": 1.0, "depth": 6}
						if obj[1]>0:
							return 'False'
						elif obj[1]<=0:
							return 'True'
						else: return 'True'
					else: return 'False'
				elif obj[3]<=1:
					return 'True'
				else: return 'True'
			elif obj[5]>8:
				return 'False'
			else: return 'False'
		else: return 'False'
	elif obj[7]>3.0:
		return 'False'
	else: return 'False'
