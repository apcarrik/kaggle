def findDecision(obj): #obj[0]: Passanger, obj[1]: Time, obj[2]: Coupon, obj[3]: Age, obj[4]: Education, obj[5]: Occupation, obj[6]: Bar, obj[7]: Coffeehouse, obj[8]: Restaurant20to50, obj[9]: Direction_same, obj[10]: Distance
	# {"feature": "Passanger", "instances": 51, "metric_value": 0.9975, "depth": 1}
	if obj[0]>1:
		# {"feature": "Education", "instances": 26, "metric_value": 0.9612, "depth": 2}
		if obj[4]>0:
			# {"feature": "Age", "instances": 21, "metric_value": 0.8631, "depth": 3}
			if obj[3]<=6:
				# {"feature": "Coupon", "instances": 20, "metric_value": 0.8113, "depth": 4}
				if obj[2]>0:
					# {"feature": "Restaurant20to50", "instances": 17, "metric_value": 0.874, "depth": 5}
					if obj[8]>0.0:
						# {"feature": "Bar", "instances": 14, "metric_value": 0.7496, "depth": 6}
						if obj[6]>0.0:
							# {"feature": "Time", "instances": 9, "metric_value": 0.9183, "depth": 7}
							if obj[1]<=2:
								# {"feature": "Occupation", "instances": 6, "metric_value": 0.65, "depth": 8}
								if obj[5]>1:
									return 'False'
								elif obj[5]<=1:
									# {"feature": "Coffeehouse", "instances": 2, "metric_value": 1.0, "depth": 9}
									if obj[7]<=1.0:
										return 'True'
									elif obj[7]>1.0:
										return 'False'
									else: return 'False'
								else: return 'True'
							elif obj[1]>2:
								# {"feature": "Occupation", "instances": 3, "metric_value": 0.9183, "depth": 8}
								if obj[5]>2:
									return 'True'
								elif obj[5]<=2:
									return 'False'
								else: return 'False'
							else: return 'True'
						elif obj[6]<=0.0:
							return 'False'
						else: return 'False'
					elif obj[8]<=0.0:
						# {"feature": "Time", "instances": 3, "metric_value": 0.9183, "depth": 6}
						if obj[1]>0:
							# {"feature": "Occupation", "instances": 2, "metric_value": 1.0, "depth": 7}
							if obj[5]<=23:
								# {"feature": "Bar", "instances": 2, "metric_value": 1.0, "depth": 8}
								if obj[6]<=0.0:
									# {"feature": "Coffeehouse", "instances": 2, "metric_value": 1.0, "depth": 9}
									if obj[7]<=0.0:
										# {"feature": "Direction_same", "instances": 2, "metric_value": 1.0, "depth": 10}
										if obj[9]<=0:
											# {"feature": "Distance", "instances": 2, "metric_value": 1.0, "depth": 11}
											if obj[10]<=2:
												return 'False'
											else: return 'False'
										else: return 'False'
									else: return 'False'
								else: return 'False'
							else: return 'False'
						elif obj[1]<=0:
							return 'True'
						else: return 'True'
					else: return 'True'
				elif obj[2]<=0:
					return 'False'
				else: return 'False'
			elif obj[3]>6:
				return 'True'
			else: return 'True'
		elif obj[4]<=0:
			# {"feature": "Age", "instances": 5, "metric_value": 0.7219, "depth": 3}
			if obj[3]>0:
				return 'True'
			elif obj[3]<=0:
				return 'False'
			else: return 'False'
		else: return 'True'
	elif obj[0]<=1:
		# {"feature": "Age", "instances": 25, "metric_value": 0.9044, "depth": 2}
		if obj[3]>1:
			# {"feature": "Occupation", "instances": 18, "metric_value": 0.9911, "depth": 3}
			if obj[5]<=11:
				# {"feature": "Time", "instances": 15, "metric_value": 0.9968, "depth": 4}
				if obj[1]<=3:
					# {"feature": "Restaurant20to50", "instances": 13, "metric_value": 0.9612, "depth": 5}
					if obj[8]<=1.0:
						# {"feature": "Education", "instances": 7, "metric_value": 0.5917, "depth": 6}
						if obj[4]>0:
							return 'False'
						elif obj[4]<=0:
							# {"feature": "Bar", "instances": 2, "metric_value": 1.0, "depth": 7}
							if obj[6]>0.0:
								return 'False'
							elif obj[6]<=0.0:
								return 'True'
							else: return 'True'
						else: return 'False'
					elif obj[8]>1.0:
						# {"feature": "Coupon", "instances": 6, "metric_value": 0.9183, "depth": 6}
						if obj[2]<=3:
							return 'True'
						elif obj[2]>3:
							# {"feature": "Coffeehouse", "instances": 3, "metric_value": 0.9183, "depth": 7}
							if obj[7]>0.0:
								return 'False'
							elif obj[7]<=0.0:
								return 'True'
							else: return 'True'
						else: return 'False'
					else: return 'True'
				elif obj[1]>3:
					return 'True'
				else: return 'True'
			elif obj[5]>11:
				return 'True'
			else: return 'True'
		elif obj[3]<=1:
			return 'True'
		else: return 'True'
	else: return 'True'
