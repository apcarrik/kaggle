def findDecision(obj): #obj[0]: Passanger, obj[1]: Time, obj[2]: Coupon, obj[3]: Age, obj[4]: Education, obj[5]: Occupation, obj[6]: Bar, obj[7]: Coffeehouse, obj[8]: Restaurant20to50, obj[9]: Direction_same, obj[10]: Distance
	# {"feature": "Coupon", "instances": 85, "metric_value": 0.9999, "depth": 1}
	if obj[2]>0:
		# {"feature": "Occupation", "instances": 75, "metric_value": 0.9937, "depth": 2}
		if obj[5]>1:
			# {"feature": "Passanger", "instances": 63, "metric_value": 0.9998, "depth": 3}
			if obj[0]<=2:
				# {"feature": "Coffeehouse", "instances": 50, "metric_value": 0.9815, "depth": 4}
				if obj[7]<=3.0:
					# {"feature": "Education", "instances": 47, "metric_value": 0.9918, "depth": 5}
					if obj[4]>0:
						# {"feature": "Bar", "instances": 29, "metric_value": 0.9923, "depth": 6}
						if obj[6]>-1.0:
							# {"feature": "Distance", "instances": 28, "metric_value": 0.9852, "depth": 7}
							if obj[10]<=2:
								# {"feature": "Time", "instances": 26, "metric_value": 0.9957, "depth": 8}
								if obj[1]>0:
									# {"feature": "Age", "instances": 18, "metric_value": 0.9641, "depth": 9}
									if obj[3]>0:
										# {"feature": "Restaurant20to50", "instances": 16, "metric_value": 0.9887, "depth": 10}
										if obj[8]<=1.0:
											# {"feature": "Direction_same", "instances": 13, "metric_value": 0.8905, "depth": 11}
											if obj[9]<=0:
												return 'True'
											elif obj[9]>0:
												return 'True'
											else: return 'True'
										elif obj[8]>1.0:
											return 'False'
										else: return 'False'
									elif obj[3]<=0:
										return 'True'
									else: return 'True'
								elif obj[1]<=0:
									# {"feature": "Age", "instances": 8, "metric_value": 0.9544, "depth": 9}
									if obj[3]<=4:
										# {"feature": "Restaurant20to50", "instances": 7, "metric_value": 0.8631, "depth": 10}
										if obj[8]<=1.0:
											# {"feature": "Direction_same", "instances": 5, "metric_value": 0.971, "depth": 11}
											if obj[9]<=0:
												return 'False'
											elif obj[9]>0:
												return 'False'
											else: return 'False'
										elif obj[8]>1.0:
											return 'False'
										else: return 'False'
									elif obj[3]>4:
										return 'True'
									else: return 'True'
								else: return 'False'
							elif obj[10]>2:
								return 'True'
							else: return 'True'
						elif obj[6]<=-1.0:
							return 'False'
						else: return 'False'
					elif obj[4]<=0:
						# {"feature": "Bar", "instances": 18, "metric_value": 0.8524, "depth": 6}
						if obj[6]<=2.0:
							# {"feature": "Direction_same", "instances": 14, "metric_value": 0.9403, "depth": 7}
							if obj[9]<=0:
								# {"feature": "Distance", "instances": 12, "metric_value": 0.8113, "depth": 8}
								if obj[10]>1:
									# {"feature": "Time", "instances": 7, "metric_value": 0.9852, "depth": 9}
									if obj[1]<=1:
										# {"feature": "Age", "instances": 6, "metric_value": 0.9183, "depth": 10}
										if obj[3]<=3:
											# {"feature": "Restaurant20to50", "instances": 3, "metric_value": 0.9183, "depth": 11}
											if obj[8]>0.0:
												return 'True'
											elif obj[8]<=0.0:
												return 'True'
											else: return 'True'
										elif obj[3]>3:
											return 'False'
										else: return 'False'
									elif obj[1]>1:
										return 'True'
									else: return 'True'
								elif obj[10]<=1:
									return 'False'
								else: return 'False'
							elif obj[9]>0:
								return 'True'
							else: return 'True'
						elif obj[6]>2.0:
							return 'False'
						else: return 'False'
					else: return 'False'
				elif obj[7]>3.0:
					return 'False'
				else: return 'False'
			elif obj[0]>2:
				# {"feature": "Distance", "instances": 13, "metric_value": 0.7793, "depth": 4}
				if obj[10]>1:
					return 'True'
				elif obj[10]<=1:
					# {"feature": "Age", "instances": 6, "metric_value": 1.0, "depth": 5}
					if obj[3]<=3:
						# {"feature": "Time", "instances": 4, "metric_value": 0.8113, "depth": 6}
						if obj[1]<=3:
							return 'False'
						elif obj[1]>3:
							return 'True'
						else: return 'True'
					elif obj[3]>3:
						return 'True'
					else: return 'True'
				else: return 'False'
			else: return 'True'
		elif obj[5]<=1:
			# {"feature": "Age", "instances": 12, "metric_value": 0.65, "depth": 3}
			if obj[3]>0:
				# {"feature": "Coffeehouse", "instances": 11, "metric_value": 0.4395, "depth": 4}
				if obj[7]<=2.0:
					return 'True'
				elif obj[7]>2.0:
					# {"feature": "Time", "instances": 3, "metric_value": 0.9183, "depth": 5}
					if obj[1]>1:
						return 'True'
					elif obj[1]<=1:
						return 'False'
					else: return 'False'
				else: return 'True'
			elif obj[3]<=0:
				return 'False'
			else: return 'False'
		else: return 'True'
	elif obj[2]<=0:
		# {"feature": "Occupation", "instances": 10, "metric_value": 0.469, "depth": 2}
		if obj[5]<=9:
			return 'False'
		elif obj[5]>9:
			# {"feature": "Passanger", "instances": 2, "metric_value": 1.0, "depth": 3}
			if obj[0]<=0:
				return 'False'
			elif obj[0]>0:
				return 'True'
			else: return 'True'
		else: return 'False'
	else: return 'False'
