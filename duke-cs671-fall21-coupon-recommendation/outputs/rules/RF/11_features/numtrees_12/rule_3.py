def findDecision(obj): #obj[0]: Passanger, obj[1]: Time, obj[2]: Coupon, obj[3]: Age, obj[4]: Education, obj[5]: Occupation, obj[6]: Bar, obj[7]: Coffeehouse, obj[8]: Restaurant20to50, obj[9]: Direction_same, obj[10]: Distance
	# {"feature": "Passanger", "instances": 85, "metric_value": 0.9143, "depth": 1}
	if obj[0]<=2:
		# {"feature": "Age", "instances": 58, "metric_value": 0.9862, "depth": 2}
		if obj[3]>2:
			# {"feature": "Coffeehouse", "instances": 32, "metric_value": 0.8571, "depth": 3}
			if obj[7]>0.0:
				# {"feature": "Coupon", "instances": 25, "metric_value": 0.7219, "depth": 4}
				if obj[2]<=3:
					# {"feature": "Distance", "instances": 14, "metric_value": 0.3712, "depth": 5}
					if obj[10]<=2:
						return 'True'
					elif obj[10]>2:
						# {"feature": "Occupation", "instances": 3, "metric_value": 0.9183, "depth": 6}
						if obj[5]<=1:
							return 'True'
						elif obj[5]>1:
							return 'False'
						else: return 'False'
					else: return 'True'
				elif obj[2]>3:
					# {"feature": "Occupation", "instances": 11, "metric_value": 0.9457, "depth": 5}
					if obj[5]>2:
						# {"feature": "Time", "instances": 8, "metric_value": 1.0, "depth": 6}
						if obj[1]>0:
							# {"feature": "Direction_same", "instances": 6, "metric_value": 0.9183, "depth": 7}
							if obj[9]>0:
								return 'True'
							elif obj[9]<=0:
								# {"feature": "Education", "instances": 3, "metric_value": 0.9183, "depth": 8}
								if obj[4]>2:
									# {"feature": "Bar", "instances": 2, "metric_value": 1.0, "depth": 9}
									if obj[6]<=0.0:
										return 'False'
									elif obj[6]>0.0:
										return 'True'
									else: return 'True'
								elif obj[4]<=2:
									return 'False'
								else: return 'False'
							else: return 'False'
						elif obj[1]<=0:
							return 'False'
						else: return 'False'
					elif obj[5]<=2:
						return 'True'
					else: return 'True'
				else: return 'True'
			elif obj[7]<=0.0:
				# {"feature": "Education", "instances": 7, "metric_value": 0.9852, "depth": 4}
				if obj[4]<=2:
					# {"feature": "Restaurant20to50", "instances": 5, "metric_value": 0.7219, "depth": 5}
					if obj[8]<=1.0:
						return 'False'
					elif obj[8]>1.0:
						return 'True'
					else: return 'True'
				elif obj[4]>2:
					return 'True'
				else: return 'True'
			else: return 'False'
		elif obj[3]<=2:
			# {"feature": "Coupon", "instances": 26, "metric_value": 0.9612, "depth": 3}
			if obj[2]>0:
				# {"feature": "Time", "instances": 21, "metric_value": 0.9984, "depth": 4}
				if obj[1]<=3:
					# {"feature": "Bar", "instances": 19, "metric_value": 0.9819, "depth": 5}
					if obj[6]>0.0:
						# {"feature": "Education", "instances": 13, "metric_value": 0.9957, "depth": 6}
						if obj[4]<=2:
							# {"feature": "Distance", "instances": 10, "metric_value": 0.971, "depth": 7}
							if obj[10]<=2:
								# {"feature": "Restaurant20to50", "instances": 7, "metric_value": 0.9852, "depth": 8}
								if obj[8]<=2.0:
									# {"feature": "Occupation", "instances": 6, "metric_value": 0.9183, "depth": 9}
									if obj[5]>12:
										# {"feature": "Coffeehouse", "instances": 4, "metric_value": 1.0, "depth": 10}
										if obj[7]>0.0:
											# {"feature": "Direction_same", "instances": 3, "metric_value": 0.9183, "depth": 11}
											if obj[9]<=0:
												return 'True'
											else: return 'True'
										elif obj[7]<=0.0:
											return 'False'
										else: return 'False'
									elif obj[5]<=12:
										return 'True'
									else: return 'True'
								elif obj[8]>2.0:
									return 'False'
								else: return 'False'
							elif obj[10]>2:
								return 'False'
							else: return 'False'
						elif obj[4]>2:
							return 'True'
						else: return 'True'
					elif obj[6]<=0.0:
						# {"feature": "Education", "instances": 6, "metric_value": 0.65, "depth": 6}
						if obj[4]>0:
							return 'False'
						elif obj[4]<=0:
							# {"feature": "Occupation", "instances": 2, "metric_value": 1.0, "depth": 7}
							if obj[5]<=1:
								# {"feature": "Coffeehouse", "instances": 2, "metric_value": 1.0, "depth": 8}
								if obj[7]<=0.0:
									# {"feature": "Restaurant20to50", "instances": 2, "metric_value": 1.0, "depth": 9}
									if obj[8]<=2.0:
										# {"feature": "Direction_same", "instances": 2, "metric_value": 1.0, "depth": 10}
										if obj[9]<=0:
											# {"feature": "Distance", "instances": 2, "metric_value": 1.0, "depth": 11}
											if obj[10]<=3:
												return 'True'
											else: return 'True'
										else: return 'True'
									else: return 'True'
								else: return 'True'
							else: return 'True'
						else: return 'True'
					else: return 'False'
				elif obj[1]>3:
					return 'True'
				else: return 'True'
			elif obj[2]<=0:
				return 'False'
			else: return 'False'
		else: return 'False'
	elif obj[0]>2:
		# {"feature": "Coupon", "instances": 27, "metric_value": 0.5033, "depth": 2}
		if obj[2]>0:
			# {"feature": "Occupation", "instances": 26, "metric_value": 0.3912, "depth": 3}
			if obj[5]<=12:
				return 'True'
			elif obj[5]>12:
				# {"feature": "Education", "instances": 5, "metric_value": 0.971, "depth": 4}
				if obj[4]<=2:
					# {"feature": "Distance", "instances": 3, "metric_value": 0.9183, "depth": 5}
					if obj[10]>1:
						return 'False'
					elif obj[10]<=1:
						return 'True'
					else: return 'True'
				elif obj[4]>2:
					return 'True'
				else: return 'True'
			else: return 'True'
		elif obj[2]<=0:
			return 'False'
		else: return 'False'
	else: return 'True'
