def findDecision(obj): #obj[0]: Passanger, obj[1]: Coupon, obj[2]: Education, obj[3]: Occupation, obj[4]: Bar, obj[5]: Restaurant20to50, obj[6]: Direction_same, obj[7]: Distance
	# {"feature": "Distance", "instances": 51, "metric_value": 0.9975, "depth": 1}
	if obj[7]<=2:
		# {"feature": "Education", "instances": 44, "metric_value": 0.9624, "depth": 2}
		if obj[2]<=1:
			# {"feature": "Bar", "instances": 26, "metric_value": 0.8404, "depth": 3}
			if obj[4]<=1.0:
				# {"feature": "Coupon", "instances": 20, "metric_value": 0.9341, "depth": 4}
				if obj[1]>0:
					# {"feature": "Occupation", "instances": 18, "metric_value": 0.8524, "depth": 5}
					if obj[3]>6:
						# {"feature": "Restaurant20to50", "instances": 12, "metric_value": 0.65, "depth": 6}
						if obj[5]>0.0:
							# {"feature": "Direction_same", "instances": 10, "metric_value": 0.7219, "depth": 7}
							if obj[6]<=0:
								# {"feature": "Passanger", "instances": 9, "metric_value": 0.7642, "depth": 8}
								if obj[0]<=1:
									return 'True'
								elif obj[0]>1:
									return 'True'
								else: return 'True'
							elif obj[6]>0:
								return 'True'
							else: return 'True'
						elif obj[5]<=0.0:
							return 'True'
						else: return 'True'
					elif obj[3]<=6:
						# {"feature": "Restaurant20to50", "instances": 6, "metric_value": 1.0, "depth": 6}
						if obj[5]<=1.0:
							# {"feature": "Passanger", "instances": 4, "metric_value": 0.8113, "depth": 7}
							if obj[0]>1:
								# {"feature": "Direction_same", "instances": 2, "metric_value": 1.0, "depth": 8}
								if obj[6]<=0:
									return 'True'
								else: return 'True'
							elif obj[0]<=1:
								return 'False'
							else: return 'False'
						elif obj[5]>1.0:
							return 'True'
						else: return 'True'
					else: return 'True'
				elif obj[1]<=0:
					return 'False'
				else: return 'False'
			elif obj[4]>1.0:
				return 'True'
			else: return 'True'
		elif obj[2]>1:
			# {"feature": "Restaurant20to50", "instances": 18, "metric_value": 0.9911, "depth": 3}
			if obj[5]>0.0:
				# {"feature": "Occupation", "instances": 16, "metric_value": 1.0, "depth": 4}
				if obj[3]<=6:
					# {"feature": "Passanger", "instances": 11, "metric_value": 0.9457, "depth": 5}
					if obj[0]>0:
						# {"feature": "Bar", "instances": 10, "metric_value": 0.8813, "depth": 6}
						if obj[4]<=2.0:
							# {"feature": "Direction_same", "instances": 9, "metric_value": 0.7642, "depth": 7}
							if obj[6]<=0:
								# {"feature": "Coupon", "instances": 8, "metric_value": 0.8113, "depth": 8}
								if obj[1]>0:
									return 'True'
								elif obj[1]<=0:
									return 'True'
								else: return 'True'
							elif obj[6]>0:
								return 'True'
							else: return 'True'
						elif obj[4]>2.0:
							return 'False'
						else: return 'False'
					elif obj[0]<=0:
						return 'False'
					else: return 'False'
				elif obj[3]>6:
					# {"feature": "Bar", "instances": 5, "metric_value": 0.7219, "depth": 5}
					if obj[4]>1.0:
						return 'False'
					elif obj[4]<=1.0:
						return 'True'
					else: return 'True'
				else: return 'False'
			elif obj[5]<=0.0:
				return 'False'
			else: return 'False'
		else: return 'False'
	elif obj[7]>2:
		return 'False'
	else: return 'False'
