def findDecision(obj): #obj[0]: Passanger, obj[1]: Coupon, obj[2]: Education, obj[3]: Occupation, obj[4]: Bar, obj[5]: Restaurant20to50, obj[6]: Direction_same, obj[7]: Distance
	# {"feature": "Occupation", "instances": 51, "metric_value": 0.9774, "depth": 1}
	if obj[3]<=20:
		# {"feature": "Restaurant20to50", "instances": 47, "metric_value": 0.9918, "depth": 2}
		if obj[5]<=2.0:
			# {"feature": "Passanger", "instances": 45, "metric_value": 0.9968, "depth": 3}
			if obj[0]<=2:
				# {"feature": "Bar", "instances": 36, "metric_value": 0.9799, "depth": 4}
				if obj[4]<=1.0:
					# {"feature": "Coupon", "instances": 27, "metric_value": 0.999, "depth": 5}
					if obj[1]>0:
						# {"feature": "Education", "instances": 22, "metric_value": 0.994, "depth": 6}
						if obj[2]<=3:
							# {"feature": "Direction_same", "instances": 21, "metric_value": 0.9852, "depth": 7}
							if obj[6]<=0:
								# {"feature": "Distance", "instances": 18, "metric_value": 1.0, "depth": 8}
								if obj[7]<=1:
									return 'True'
								elif obj[7]>1:
									return 'False'
								else: return 'False'
							elif obj[6]>0:
								return 'True'
							else: return 'True'
						elif obj[2]>3:
							return 'False'
						else: return 'False'
					elif obj[1]<=0:
						# {"feature": "Direction_same", "instances": 5, "metric_value": 0.7219, "depth": 6}
						if obj[6]<=0:
							return 'False'
						elif obj[6]>0:
							# {"feature": "Education", "instances": 2, "metric_value": 1.0, "depth": 7}
							if obj[2]<=2:
								# {"feature": "Distance", "instances": 2, "metric_value": 1.0, "depth": 8}
								if obj[7]<=1:
									return 'False'
								else: return 'False'
							else: return 'False'
						else: return 'False'
					else: return 'False'
				elif obj[4]>1.0:
					# {"feature": "Coupon", "instances": 9, "metric_value": 0.7642, "depth": 5}
					if obj[1]<=3:
						# {"feature": "Education", "instances": 6, "metric_value": 0.9183, "depth": 6}
						if obj[2]>0:
							# {"feature": "Direction_same", "instances": 4, "metric_value": 1.0, "depth": 7}
							if obj[6]>0:
								# {"feature": "Distance", "instances": 2, "metric_value": 1.0, "depth": 8}
								if obj[7]<=2:
									return 'False'
								else: return 'False'
							elif obj[6]<=0:
								# {"feature": "Distance", "instances": 2, "metric_value": 1.0, "depth": 8}
								if obj[7]<=3:
									return 'True'
								else: return 'True'
							else: return 'True'
						elif obj[2]<=0:
							return 'False'
						else: return 'False'
					elif obj[1]>3:
						return 'False'
					else: return 'False'
				else: return 'False'
			elif obj[0]>2:
				# {"feature": "Coupon", "instances": 9, "metric_value": 0.9183, "depth": 4}
				if obj[1]>0:
					# {"feature": "Education", "instances": 8, "metric_value": 0.9544, "depth": 5}
					if obj[2]>0:
						# {"feature": "Bar", "instances": 7, "metric_value": 0.8631, "depth": 6}
						if obj[4]<=1.0:
							# {"feature": "Distance", "instances": 5, "metric_value": 0.971, "depth": 7}
							if obj[7]>1:
								# {"feature": "Direction_same", "instances": 4, "metric_value": 1.0, "depth": 8}
								if obj[6]<=0:
									return 'False'
								else: return 'False'
							elif obj[7]<=1:
								return 'True'
							else: return 'True'
						elif obj[4]>1.0:
							return 'True'
						else: return 'True'
					elif obj[2]<=0:
						return 'False'
					else: return 'False'
				elif obj[1]<=0:
					return 'True'
				else: return 'True'
			else: return 'True'
		elif obj[5]>2.0:
			return 'False'
		else: return 'False'
	elif obj[3]>20:
		return 'False'
	else: return 'False'
