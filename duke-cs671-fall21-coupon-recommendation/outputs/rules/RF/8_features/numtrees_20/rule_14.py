def findDecision(obj): #obj[0]: Passanger, obj[1]: Coupon, obj[2]: Education, obj[3]: Occupation, obj[4]: Bar, obj[5]: Restaurant20to50, obj[6]: Direction_same, obj[7]: Distance
	# {"feature": "Coupon", "instances": 51, "metric_value": 0.9975, "depth": 1}
	if obj[1]>0:
		# {"feature": "Distance", "instances": 41, "metric_value": 0.9892, "depth": 2}
		if obj[7]<=1:
			# {"feature": "Education", "instances": 25, "metric_value": 0.9044, "depth": 3}
			if obj[2]<=3:
				# {"feature": "Occupation", "instances": 22, "metric_value": 0.9457, "depth": 4}
				if obj[3]<=12:
					# {"feature": "Restaurant20to50", "instances": 21, "metric_value": 0.9183, "depth": 5}
					if obj[5]>0.0:
						# {"feature": "Passanger", "instances": 16, "metric_value": 0.8113, "depth": 6}
						if obj[0]<=1:
							# {"feature": "Direction_same", "instances": 11, "metric_value": 0.9457, "depth": 7}
							if obj[6]>0:
								# {"feature": "Bar", "instances": 8, "metric_value": 0.8113, "depth": 8}
								if obj[4]>0.0:
									return 'True'
								elif obj[4]<=0.0:
									return 'True'
								else: return 'True'
							elif obj[6]<=0:
								# {"feature": "Bar", "instances": 3, "metric_value": 0.9183, "depth": 8}
								if obj[4]<=0.0:
									return 'True'
								elif obj[4]>0.0:
									return 'False'
								else: return 'False'
							else: return 'False'
						elif obj[0]>1:
							return 'True'
						else: return 'True'
					elif obj[5]<=0.0:
						# {"feature": "Passanger", "instances": 5, "metric_value": 0.971, "depth": 6}
						if obj[0]<=1:
							# {"feature": "Bar", "instances": 4, "metric_value": 1.0, "depth": 7}
							if obj[4]<=1.0:
								# {"feature": "Direction_same", "instances": 3, "metric_value": 0.9183, "depth": 8}
								if obj[6]<=0:
									return 'True'
								elif obj[6]>0:
									return 'True'
								else: return 'True'
							elif obj[4]>1.0:
								return 'False'
							else: return 'False'
						elif obj[0]>1:
							return 'False'
						else: return 'False'
					else: return 'False'
				elif obj[3]>12:
					return 'False'
				else: return 'False'
			elif obj[2]>3:
				return 'True'
			else: return 'True'
		elif obj[7]>1:
			# {"feature": "Occupation", "instances": 16, "metric_value": 0.9544, "depth": 3}
			if obj[3]>0:
				# {"feature": "Education", "instances": 15, "metric_value": 0.9183, "depth": 4}
				if obj[2]>0:
					# {"feature": "Bar", "instances": 12, "metric_value": 0.9799, "depth": 5}
					if obj[4]<=2.0:
						# {"feature": "Restaurant20to50", "instances": 11, "metric_value": 0.994, "depth": 6}
						if obj[5]>0.0:
							# {"feature": "Passanger", "instances": 8, "metric_value": 0.9544, "depth": 7}
							if obj[0]<=2:
								# {"feature": "Direction_same", "instances": 6, "metric_value": 0.9183, "depth": 8}
								if obj[6]<=0:
									return 'False'
								else: return 'False'
							elif obj[0]>2:
								# {"feature": "Direction_same", "instances": 2, "metric_value": 1.0, "depth": 8}
								if obj[6]<=0:
									return 'True'
								else: return 'True'
							else: return 'True'
						elif obj[5]<=0.0:
							# {"feature": "Passanger", "instances": 3, "metric_value": 0.9183, "depth": 7}
							if obj[0]<=1:
								# {"feature": "Direction_same", "instances": 3, "metric_value": 0.9183, "depth": 8}
								if obj[6]<=0:
									return 'True'
								else: return 'True'
							else: return 'True'
						else: return 'True'
					elif obj[4]>2.0:
						return 'False'
					else: return 'False'
				elif obj[2]<=0:
					return 'False'
				else: return 'False'
			elif obj[3]<=0:
				return 'True'
			else: return 'True'
		else: return 'False'
	elif obj[1]<=0:
		# {"feature": "Bar", "instances": 10, "metric_value": 0.469, "depth": 2}
		if obj[4]<=1.0:
			return 'False'
		elif obj[4]>1.0:
			return 'True'
		else: return 'True'
	else: return 'False'
