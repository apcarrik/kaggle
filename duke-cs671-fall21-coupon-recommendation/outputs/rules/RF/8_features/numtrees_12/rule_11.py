def findDecision(obj): #obj[0]: Passanger, obj[1]: Coupon, obj[2]: Education, obj[3]: Occupation, obj[4]: Bar, obj[5]: Restaurant20to50, obj[6]: Direction_same, obj[7]: Distance
	# {"feature": "Coupon", "instances": 85, "metric_value": 0.9999, "depth": 1}
	if obj[1]>1:
		# {"feature": "Bar", "instances": 68, "metric_value": 0.9843, "depth": 2}
		if obj[4]>0.0:
			# {"feature": "Distance", "instances": 39, "metric_value": 0.9995, "depth": 3}
			if obj[7]<=1:
				# {"feature": "Education", "instances": 22, "metric_value": 0.9457, "depth": 4}
				if obj[2]>1:
					# {"feature": "Passanger", "instances": 16, "metric_value": 0.9887, "depth": 5}
					if obj[0]<=2:
						# {"feature": "Occupation", "instances": 12, "metric_value": 0.9183, "depth": 6}
						if obj[3]>2:
							# {"feature": "Restaurant20to50", "instances": 9, "metric_value": 0.9911, "depth": 7}
							if obj[5]>0.0:
								# {"feature": "Direction_same", "instances": 8, "metric_value": 1.0, "depth": 8}
								if obj[6]>0:
									return 'True'
								elif obj[6]<=0:
									return 'False'
								else: return 'False'
							elif obj[5]<=0.0:
								return 'True'
							else: return 'True'
						elif obj[3]<=2:
							return 'True'
						else: return 'True'
					elif obj[0]>2:
						# {"feature": "Occupation", "instances": 4, "metric_value": 0.8113, "depth": 6}
						if obj[3]<=5:
							return 'False'
						elif obj[3]>5:
							return 'True'
						else: return 'True'
					else: return 'False'
				elif obj[2]<=1:
					# {"feature": "Occupation", "instances": 6, "metric_value": 0.65, "depth": 5}
					if obj[3]>2:
						return 'True'
					elif obj[3]<=2:
						return 'False'
					else: return 'False'
				else: return 'True'
			elif obj[7]>1:
				# {"feature": "Occupation", "instances": 17, "metric_value": 0.874, "depth": 4}
				if obj[3]>6:
					# {"feature": "Passanger", "instances": 11, "metric_value": 0.994, "depth": 5}
					if obj[0]>0:
						# {"feature": "Direction_same", "instances": 10, "metric_value": 0.971, "depth": 6}
						if obj[6]<=0:
							# {"feature": "Education", "instances": 9, "metric_value": 0.9183, "depth": 7}
							if obj[2]>0:
								# {"feature": "Restaurant20to50", "instances": 5, "metric_value": 0.7219, "depth": 8}
								if obj[5]>1.0:
									return 'False'
								elif obj[5]<=1.0:
									return 'False'
								else: return 'False'
							elif obj[2]<=0:
								# {"feature": "Restaurant20to50", "instances": 4, "metric_value": 1.0, "depth": 8}
								if obj[5]>1.0:
									return 'False'
								elif obj[5]<=1.0:
									return 'True'
								else: return 'True'
							else: return 'False'
						elif obj[6]>0:
							return 'True'
						else: return 'True'
					elif obj[0]<=0:
						return 'True'
					else: return 'True'
				elif obj[3]<=6:
					return 'False'
				else: return 'False'
			else: return 'False'
		elif obj[4]<=0.0:
			# {"feature": "Education", "instances": 29, "metric_value": 0.8936, "depth": 3}
			if obj[2]<=2:
				# {"feature": "Occupation", "instances": 26, "metric_value": 0.9306, "depth": 4}
				if obj[3]<=7:
					# {"feature": "Distance", "instances": 19, "metric_value": 0.8315, "depth": 5}
					if obj[7]<=2:
						# {"feature": "Passanger", "instances": 14, "metric_value": 0.9403, "depth": 6}
						if obj[0]>1:
							# {"feature": "Restaurant20to50", "instances": 8, "metric_value": 0.5436, "depth": 7}
							if obj[5]<=0.0:
								# {"feature": "Direction_same", "instances": 4, "metric_value": 0.8113, "depth": 8}
								if obj[6]<=0:
									return 'True'
								else: return 'True'
							elif obj[5]>0.0:
								return 'True'
							else: return 'True'
						elif obj[0]<=1:
							# {"feature": "Restaurant20to50", "instances": 6, "metric_value": 0.9183, "depth": 7}
							if obj[5]>0.0:
								return 'False'
							elif obj[5]<=0.0:
								# {"feature": "Direction_same", "instances": 3, "metric_value": 0.9183, "depth": 8}
								if obj[6]<=0:
									return 'True'
								elif obj[6]>0:
									return 'True'
								else: return 'True'
							else: return 'True'
						else: return 'False'
					elif obj[7]>2:
						return 'True'
					else: return 'True'
				elif obj[3]>7:
					# {"feature": "Distance", "instances": 7, "metric_value": 0.9852, "depth": 5}
					if obj[7]<=1:
						# {"feature": "Passanger", "instances": 5, "metric_value": 0.971, "depth": 6}
						if obj[0]<=1:
							# {"feature": "Restaurant20to50", "instances": 4, "metric_value": 0.8113, "depth": 7}
							if obj[5]>0.0:
								# {"feature": "Direction_same", "instances": 3, "metric_value": 0.9183, "depth": 8}
								if obj[6]<=0:
									return 'True'
								elif obj[6]>0:
									return 'True'
								else: return 'True'
							elif obj[5]<=0.0:
								return 'True'
							else: return 'True'
						elif obj[0]>1:
							return 'False'
						else: return 'False'
					elif obj[7]>1:
						return 'False'
					else: return 'False'
				else: return 'False'
			elif obj[2]>2:
				return 'True'
			else: return 'True'
		else: return 'True'
	elif obj[1]<=1:
		# {"feature": "Occupation", "instances": 17, "metric_value": 0.7871, "depth": 2}
		if obj[3]>5:
			# {"feature": "Restaurant20to50", "instances": 12, "metric_value": 0.9183, "depth": 3}
			if obj[5]<=1.0:
				# {"feature": "Passanger", "instances": 9, "metric_value": 0.9911, "depth": 4}
				if obj[0]<=1:
					# {"feature": "Education", "instances": 8, "metric_value": 0.9544, "depth": 5}
					if obj[2]<=2:
						# {"feature": "Distance", "instances": 7, "metric_value": 0.9852, "depth": 6}
						if obj[7]<=2:
							# {"feature": "Bar", "instances": 6, "metric_value": 1.0, "depth": 7}
							if obj[4]<=2.0:
								# {"feature": "Direction_same", "instances": 5, "metric_value": 0.971, "depth": 8}
								if obj[6]<=0:
									return 'False'
								elif obj[6]>0:
									return 'False'
								else: return 'False'
							elif obj[4]>2.0:
								return 'True'
							else: return 'True'
						elif obj[7]>2:
							return 'False'
						else: return 'False'
					elif obj[2]>2:
						return 'False'
					else: return 'False'
				elif obj[0]>1:
					return 'True'
				else: return 'True'
			elif obj[5]>1.0:
				return 'False'
			else: return 'False'
		elif obj[3]<=5:
			return 'False'
		else: return 'False'
	else: return 'False'
