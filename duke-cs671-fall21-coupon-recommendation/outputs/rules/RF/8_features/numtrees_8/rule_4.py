def findDecision(obj): #obj[0]: Passanger, obj[1]: Coupon, obj[2]: Education, obj[3]: Occupation, obj[4]: Bar, obj[5]: Restaurant20to50, obj[6]: Direction_same, obj[7]: Distance
	# {"feature": "Coupon", "instances": 127, "metric_value": 0.9946, "depth": 1}
	if obj[1]>1:
		# {"feature": "Passanger", "instances": 96, "metric_value": 0.9618, "depth": 2}
		if obj[0]>0:
			# {"feature": "Distance", "instances": 92, "metric_value": 0.9722, "depth": 3}
			if obj[7]<=2:
				# {"feature": "Education", "instances": 82, "metric_value": 0.9474, "depth": 4}
				if obj[2]>1:
					# {"feature": "Direction_same", "instances": 47, "metric_value": 0.9918, "depth": 5}
					if obj[6]<=0:
						# {"feature": "Bar", "instances": 35, "metric_value": 0.9994, "depth": 6}
						if obj[4]<=2.0:
							# {"feature": "Occupation", "instances": 29, "metric_value": 0.9784, "depth": 7}
							if obj[3]<=16:
								# {"feature": "Restaurant20to50", "instances": 28, "metric_value": 0.9666, "depth": 8}
								if obj[5]>0.0:
									return 'False'
								elif obj[5]<=0.0:
									return 'False'
								else: return 'False'
							elif obj[3]>16:
								return 'True'
							else: return 'True'
						elif obj[4]>2.0:
							# {"feature": "Occupation", "instances": 6, "metric_value": 0.65, "depth": 7}
							if obj[3]<=7:
								return 'True'
							elif obj[3]>7:
								return 'False'
							else: return 'False'
						else: return 'True'
					elif obj[6]>0:
						# {"feature": "Occupation", "instances": 12, "metric_value": 0.8113, "depth": 6}
						if obj[3]>4:
							return 'True'
						elif obj[3]<=4:
							# {"feature": "Bar", "instances": 4, "metric_value": 0.8113, "depth": 7}
							if obj[4]<=0.0:
								# {"feature": "Restaurant20to50", "instances": 2, "metric_value": 1.0, "depth": 8}
								if obj[5]<=1.0:
									return 'True'
								else: return 'True'
							elif obj[4]>0.0:
								return 'False'
							else: return 'False'
						else: return 'False'
					else: return 'True'
				elif obj[2]<=1:
					# {"feature": "Restaurant20to50", "instances": 35, "metric_value": 0.8224, "depth": 5}
					if obj[5]>0.0:
						# {"feature": "Occupation", "instances": 30, "metric_value": 0.8813, "depth": 6}
						if obj[3]<=7:
							# {"feature": "Direction_same", "instances": 22, "metric_value": 0.7732, "depth": 7}
							if obj[6]<=0:
								# {"feature": "Bar", "instances": 20, "metric_value": 0.8113, "depth": 8}
								if obj[4]<=2.0:
									return 'True'
								elif obj[4]>2.0:
									return 'True'
								else: return 'True'
							elif obj[6]>0:
								return 'True'
							else: return 'True'
						elif obj[3]>7:
							# {"feature": "Direction_same", "instances": 8, "metric_value": 1.0, "depth": 7}
							if obj[6]<=0:
								# {"feature": "Bar", "instances": 6, "metric_value": 0.9183, "depth": 8}
								if obj[4]>0.0:
									return 'True'
								elif obj[4]<=0.0:
									return 'False'
								else: return 'False'
							elif obj[6]>0:
								return 'False'
							else: return 'False'
						else: return 'True'
					elif obj[5]<=0.0:
						return 'True'
					else: return 'True'
				else: return 'True'
			elif obj[7]>2:
				# {"feature": "Education", "instances": 10, "metric_value": 0.8813, "depth": 4}
				if obj[2]>1:
					# {"feature": "Occupation", "instances": 6, "metric_value": 1.0, "depth": 5}
					if obj[3]>5:
						# {"feature": "Bar", "instances": 4, "metric_value": 0.8113, "depth": 6}
						if obj[4]>2.0:
							return 'False'
						elif obj[4]<=2.0:
							# {"feature": "Restaurant20to50", "instances": 2, "metric_value": 1.0, "depth": 7}
							if obj[5]<=1.0:
								# {"feature": "Direction_same", "instances": 2, "metric_value": 1.0, "depth": 8}
								if obj[6]<=0:
									return 'True'
								else: return 'True'
							else: return 'True'
						else: return 'True'
					elif obj[3]<=5:
						return 'True'
					else: return 'True'
				elif obj[2]<=1:
					return 'False'
				else: return 'False'
			else: return 'False'
		elif obj[0]<=0:
			return 'True'
		else: return 'True'
	elif obj[1]<=1:
		# {"feature": "Bar", "instances": 31, "metric_value": 0.9072, "depth": 2}
		if obj[4]>0.0:
			# {"feature": "Occupation", "instances": 16, "metric_value": 0.9887, "depth": 3}
			if obj[3]>2:
				# {"feature": "Passanger", "instances": 12, "metric_value": 0.9799, "depth": 4}
				if obj[0]<=1:
					# {"feature": "Education", "instances": 9, "metric_value": 0.7642, "depth": 5}
					if obj[2]<=3:
						# {"feature": "Distance", "instances": 8, "metric_value": 0.5436, "depth": 6}
						if obj[7]>1:
							return 'False'
						elif obj[7]<=1:
							# {"feature": "Restaurant20to50", "instances": 3, "metric_value": 0.9183, "depth": 7}
							if obj[5]<=1.0:
								return 'False'
							elif obj[5]>1.0:
								return 'True'
							else: return 'True'
						else: return 'False'
					elif obj[2]>3:
						return 'True'
					else: return 'True'
				elif obj[0]>1:
					return 'True'
				else: return 'True'
			elif obj[3]<=2:
				return 'True'
			else: return 'True'
		elif obj[4]<=0.0:
			# {"feature": "Occupation", "instances": 15, "metric_value": 0.3534, "depth": 3}
			if obj[3]<=7:
				return 'False'
			elif obj[3]>7:
				# {"feature": "Education", "instances": 7, "metric_value": 0.5917, "depth": 4}
				if obj[2]>1:
					# {"feature": "Passanger", "instances": 4, "metric_value": 0.8113, "depth": 5}
					if obj[0]>0:
						# {"feature": "Distance", "instances": 3, "metric_value": 0.9183, "depth": 6}
						if obj[7]>1:
							# {"feature": "Restaurant20to50", "instances": 2, "metric_value": 1.0, "depth": 7}
							if obj[5]<=1.0:
								# {"feature": "Direction_same", "instances": 2, "metric_value": 1.0, "depth": 8}
								if obj[6]<=0:
									return 'True'
								else: return 'True'
							else: return 'True'
						elif obj[7]<=1:
							return 'False'
						else: return 'False'
					elif obj[0]<=0:
						return 'False'
					else: return 'False'
				elif obj[2]<=1:
					return 'False'
				else: return 'False'
			else: return 'False'
		else: return 'False'
	else: return 'False'
