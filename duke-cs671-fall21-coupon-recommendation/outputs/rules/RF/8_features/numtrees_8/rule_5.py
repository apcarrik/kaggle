def findDecision(obj): #obj[0]: Passanger, obj[1]: Coupon, obj[2]: Education, obj[3]: Occupation, obj[4]: Bar, obj[5]: Restaurant20to50, obj[6]: Direction_same, obj[7]: Distance
	# {"feature": "Coupon", "instances": 127, "metric_value": 0.9802, "depth": 1}
	if obj[1]>1:
		# {"feature": "Education", "instances": 101, "metric_value": 0.9309, "depth": 2}
		if obj[2]<=2:
			# {"feature": "Passanger", "instances": 76, "metric_value": 0.8315, "depth": 3}
			if obj[0]<=1:
				# {"feature": "Direction_same", "instances": 51, "metric_value": 0.9367, "depth": 4}
				if obj[6]<=0:
					# {"feature": "Occupation", "instances": 35, "metric_value": 0.9994, "depth": 5}
					if obj[3]<=8:
						# {"feature": "Restaurant20to50", "instances": 25, "metric_value": 0.9427, "depth": 6}
						if obj[5]<=1.0:
							# {"feature": "Distance", "instances": 20, "metric_value": 0.8113, "depth": 7}
							if obj[7]<=2:
								# {"feature": "Bar", "instances": 15, "metric_value": 0.7219, "depth": 8}
								if obj[4]<=0.0:
									return 'True'
								elif obj[4]>0.0:
									return 'True'
								else: return 'True'
							elif obj[7]>2:
								# {"feature": "Bar", "instances": 5, "metric_value": 0.971, "depth": 8}
								if obj[4]>0.0:
									return 'False'
								elif obj[4]<=0.0:
									return 'True'
								else: return 'True'
							else: return 'True'
						elif obj[5]>1.0:
							# {"feature": "Distance", "instances": 5, "metric_value": 0.7219, "depth": 7}
							if obj[7]<=2:
								return 'False'
							elif obj[7]>2:
								# {"feature": "Bar", "instances": 2, "metric_value": 1.0, "depth": 8}
								if obj[4]<=0.0:
									return 'True'
								else: return 'True'
							else: return 'True'
						else: return 'False'
					elif obj[3]>8:
						# {"feature": "Restaurant20to50", "instances": 10, "metric_value": 0.7219, "depth": 6}
						if obj[5]<=1.0:
							return 'False'
						elif obj[5]>1.0:
							return 'True'
						else: return 'True'
					else: return 'False'
				elif obj[6]>0:
					# {"feature": "Restaurant20to50", "instances": 16, "metric_value": 0.3373, "depth": 5}
					if obj[5]>0.0:
						return 'True'
					elif obj[5]<=0.0:
						# {"feature": "Occupation", "instances": 5, "metric_value": 0.7219, "depth": 6}
						if obj[3]<=6:
							return 'True'
						elif obj[3]>6:
							# {"feature": "Bar", "instances": 2, "metric_value": 1.0, "depth": 7}
							if obj[4]<=0.0:
								return 'False'
							elif obj[4]>0.0:
								return 'True'
							else: return 'True'
						else: return 'False'
					else: return 'True'
				else: return 'True'
			elif obj[0]>1:
				# {"feature": "Occupation", "instances": 25, "metric_value": 0.4022, "depth": 4}
				if obj[3]<=19:
					# {"feature": "Distance", "instances": 24, "metric_value": 0.2499, "depth": 5}
					if obj[7]>1:
						return 'True'
					elif obj[7]<=1:
						# {"feature": "Bar", "instances": 11, "metric_value": 0.4395, "depth": 6}
						if obj[4]>0.0:
							return 'True'
						elif obj[4]<=0.0:
							# {"feature": "Restaurant20to50", "instances": 4, "metric_value": 0.8113, "depth": 7}
							if obj[5]<=1.0:
								# {"feature": "Direction_same", "instances": 2, "metric_value": 1.0, "depth": 8}
								if obj[6]<=0:
									return 'True'
								else: return 'True'
							elif obj[5]>1.0:
								return 'True'
							else: return 'True'
						else: return 'True'
					else: return 'True'
				elif obj[3]>19:
					return 'False'
				else: return 'False'
			else: return 'True'
		elif obj[2]>2:
			# {"feature": "Restaurant20to50", "instances": 25, "metric_value": 0.971, "depth": 3}
			if obj[5]<=1.0:
				# {"feature": "Distance", "instances": 18, "metric_value": 1.0, "depth": 4}
				if obj[7]>1:
					# {"feature": "Occupation", "instances": 9, "metric_value": 0.9183, "depth": 5}
					if obj[3]<=7:
						# {"feature": "Bar", "instances": 6, "metric_value": 1.0, "depth": 6}
						if obj[4]<=1.0:
							# {"feature": "Passanger", "instances": 5, "metric_value": 0.971, "depth": 7}
							if obj[0]<=1:
								# {"feature": "Direction_same", "instances": 3, "metric_value": 0.9183, "depth": 8}
								if obj[6]<=0:
									return 'False'
								else: return 'False'
							elif obj[0]>1:
								# {"feature": "Direction_same", "instances": 2, "metric_value": 1.0, "depth": 8}
								if obj[6]<=0:
									return 'True'
								else: return 'True'
							else: return 'True'
						elif obj[4]>1.0:
							return 'True'
						else: return 'True'
					elif obj[3]>7:
						return 'True'
					else: return 'True'
				elif obj[7]<=1:
					# {"feature": "Occupation", "instances": 9, "metric_value": 0.9183, "depth": 5}
					if obj[3]>5:
						# {"feature": "Bar", "instances": 6, "metric_value": 1.0, "depth": 6}
						if obj[4]>0.0:
							# {"feature": "Passanger", "instances": 4, "metric_value": 0.8113, "depth": 7}
							if obj[0]>0:
								return 'False'
							elif obj[0]<=0:
								return 'True'
							else: return 'True'
						elif obj[4]<=0.0:
							return 'True'
						else: return 'True'
					elif obj[3]<=5:
						return 'False'
					else: return 'False'
				else: return 'False'
			elif obj[5]>1.0:
				# {"feature": "Direction_same", "instances": 7, "metric_value": 0.5917, "depth": 4}
				if obj[6]<=0:
					return 'False'
				elif obj[6]>0:
					return 'True'
				else: return 'True'
			else: return 'False'
		else: return 'False'
	elif obj[1]<=1:
		# {"feature": "Bar", "instances": 26, "metric_value": 0.8905, "depth": 2}
		if obj[4]<=2.0:
			# {"feature": "Occupation", "instances": 23, "metric_value": 0.7554, "depth": 3}
			if obj[3]>2:
				# {"feature": "Restaurant20to50", "instances": 18, "metric_value": 0.5033, "depth": 4}
				if obj[5]<=1.0:
					return 'False'
				elif obj[5]>1.0:
					# {"feature": "Passanger", "instances": 4, "metric_value": 1.0, "depth": 5}
					if obj[0]>0:
						# {"feature": "Education", "instances": 3, "metric_value": 0.9183, "depth": 6}
						if obj[2]<=0:
							# {"feature": "Direction_same", "instances": 2, "metric_value": 1.0, "depth": 7}
							if obj[6]<=0:
								# {"feature": "Distance", "instances": 2, "metric_value": 1.0, "depth": 8}
								if obj[7]<=2:
									return 'False'
								else: return 'False'
							else: return 'False'
						elif obj[2]>0:
							return 'True'
						else: return 'True'
					elif obj[0]<=0:
						return 'False'
					else: return 'False'
				else: return 'False'
			elif obj[3]<=2:
				# {"feature": "Education", "instances": 5, "metric_value": 0.971, "depth": 4}
				if obj[2]<=0:
					return 'True'
				elif obj[2]>0:
					return 'False'
				else: return 'False'
			else: return 'True'
		elif obj[4]>2.0:
			return 'True'
		else: return 'True'
	else: return 'False'
