def findDecision(obj): #obj[0]: Passanger, obj[1]: Weather, obj[2]: Time, obj[3]: Coupon, obj[4]: Coupon_validity, obj[5]: Gender, obj[6]: Age, obj[7]: Maritalstatus, obj[8]: Children, obj[9]: Education, obj[10]: Occupation, obj[11]: Income, obj[12]: Bar, obj[13]: Coffeehouse, obj[14]: Restaurantlessthan20, obj[15]: Restaurant20to50, obj[16]: Direction_same, obj[17]: Distance
	# {"feature": "Weather", "instances": 51, "metric_value": 0.9997, "depth": 1}
	if obj[1]<=0:
		# {"feature": "Age", "instances": 41, "metric_value": 0.9789, "depth": 2}
		if obj[6]>0:
			# {"feature": "Bar", "instances": 34, "metric_value": 1.0, "depth": 3}
			if obj[12]>0.0:
				# {"feature": "Coupon", "instances": 17, "metric_value": 0.874, "depth": 4}
				if obj[3]<=2:
					# {"feature": "Income", "instances": 11, "metric_value": 0.4395, "depth": 5}
					if obj[11]>0:
						return 'False'
					elif obj[11]<=0:
						# {"feature": "Education", "instances": 3, "metric_value": 0.9183, "depth": 6}
						if obj[9]>0:
							return 'False'
						elif obj[9]<=0:
							return 'True'
						else: return 'True'
					else: return 'False'
				elif obj[3]>2:
					# {"feature": "Gender", "instances": 6, "metric_value": 0.9183, "depth": 5}
					if obj[5]>0:
						# {"feature": "Time", "instances": 3, "metric_value": 0.9183, "depth": 6}
						if obj[2]>0:
							return 'False'
						elif obj[2]<=0:
							return 'True'
						else: return 'True'
					elif obj[5]<=0:
						return 'True'
					else: return 'True'
				else: return 'True'
			elif obj[12]<=0.0:
				# {"feature": "Coupon", "instances": 17, "metric_value": 0.874, "depth": 4}
				if obj[3]>0:
					# {"feature": "Maritalstatus", "instances": 16, "metric_value": 0.8113, "depth": 5}
					if obj[7]<=2:
						# {"feature": "Occupation", "instances": 15, "metric_value": 0.7219, "depth": 6}
						if obj[10]<=4:
							# {"feature": "Time", "instances": 8, "metric_value": 0.9544, "depth": 7}
							if obj[2]>1:
								# {"feature": "Gender", "instances": 6, "metric_value": 0.65, "depth": 8}
								if obj[5]>0:
									return 'True'
								elif obj[5]<=0:
									# {"feature": "Passanger", "instances": 2, "metric_value": 1.0, "depth": 9}
									if obj[0]<=0:
										return 'True'
									elif obj[0]>0:
										return 'False'
									else: return 'False'
								else: return 'True'
							elif obj[2]<=1:
								return 'False'
							else: return 'False'
						elif obj[10]>4:
							return 'True'
						else: return 'True'
					elif obj[7]>2:
						return 'False'
					else: return 'False'
				elif obj[3]<=0:
					return 'False'
				else: return 'False'
			else: return 'True'
		elif obj[6]<=0:
			return 'True'
		else: return 'True'
	elif obj[1]>0:
		# {"feature": "Time", "instances": 10, "metric_value": 0.7219, "depth": 2}
		if obj[2]<=3:
			# {"feature": "Education", "instances": 9, "metric_value": 0.5033, "depth": 3}
			if obj[9]>0:
				return 'False'
			elif obj[9]<=0:
				return 'True'
			else: return 'True'
		elif obj[2]>3:
			return 'True'
		else: return 'True'
	else: return 'False'
