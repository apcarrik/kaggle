def findDecision(obj): #obj[0]: Passanger, obj[1]: Weather, obj[2]: Time, obj[3]: Coupon, obj[4]: Coupon_validity, obj[5]: Gender, obj[6]: Age, obj[7]: Maritalstatus, obj[8]: Children, obj[9]: Education, obj[10]: Occupation, obj[11]: Income, obj[12]: Bar, obj[13]: Coffeehouse, obj[14]: Restaurantlessthan20, obj[15]: Restaurant20to50, obj[16]: Direction_same, obj[17]: Distance
	# {"feature": "Coffeehouse", "instances": 51, "metric_value": 0.9662, "depth": 1}
	if obj[13]<=3.0:
		# {"feature": "Time", "instances": 47, "metric_value": 0.9252, "depth": 2}
		if obj[2]>1:
			# {"feature": "Income", "instances": 28, "metric_value": 0.9963, "depth": 3}
			if obj[11]>0:
				# {"feature": "Education", "instances": 25, "metric_value": 0.9988, "depth": 4}
				if obj[9]<=2:
					# {"feature": "Age", "instances": 17, "metric_value": 0.9774, "depth": 5}
					if obj[6]<=4:
						# {"feature": "Bar", "instances": 15, "metric_value": 0.9183, "depth": 6}
						if obj[12]<=2.0:
							# {"feature": "Restaurantlessthan20", "instances": 14, "metric_value": 0.8631, "depth": 7}
							if obj[14]<=2.0:
								# {"feature": "Maritalstatus", "instances": 10, "metric_value": 0.971, "depth": 8}
								if obj[7]<=1:
									# {"feature": "Passanger", "instances": 8, "metric_value": 1.0, "depth": 9}
									if obj[0]>0:
										# {"feature": "Restaurant20to50", "instances": 7, "metric_value": 0.9852, "depth": 10}
										if obj[15]>-1.0:
											# {"feature": "Occupation", "instances": 6, "metric_value": 0.9183, "depth": 11}
											if obj[10]<=6:
												# {"feature": "Coupon", "instances": 4, "metric_value": 1.0, "depth": 12}
												if obj[3]>2:
													# {"feature": "Distance", "instances": 3, "metric_value": 0.9183, "depth": 13}
													if obj[17]>1:
														return 'False'
													elif obj[17]<=1:
														return 'True'
													else: return 'True'
												elif obj[3]<=2:
													return 'True'
												else: return 'True'
											elif obj[10]>6:
												return 'True'
											else: return 'True'
										elif obj[15]<=-1.0:
											return 'False'
										else: return 'False'
									elif obj[0]<=0:
										return 'False'
									else: return 'False'
								elif obj[7]>1:
									return 'True'
								else: return 'True'
							elif obj[14]>2.0:
								return 'True'
							else: return 'True'
						elif obj[12]>2.0:
							return 'False'
						else: return 'False'
					elif obj[6]>4:
						return 'False'
					else: return 'False'
				elif obj[9]>2:
					# {"feature": "Passanger", "instances": 8, "metric_value": 0.8113, "depth": 5}
					if obj[0]<=2:
						return 'False'
					elif obj[0]>2:
						# {"feature": "Coupon", "instances": 4, "metric_value": 1.0, "depth": 6}
						if obj[3]>2:
							# {"feature": "Bar", "instances": 3, "metric_value": 0.9183, "depth": 7}
							if obj[12]>0.0:
								return 'True'
							elif obj[12]<=0.0:
								return 'False'
							else: return 'False'
						elif obj[3]<=2:
							return 'False'
						else: return 'False'
					else: return 'True'
				else: return 'False'
			elif obj[11]<=0:
				return 'True'
			else: return 'True'
		elif obj[2]<=1:
			# {"feature": "Gender", "instances": 19, "metric_value": 0.6292, "depth": 3}
			if obj[5]>0:
				return 'True'
			elif obj[5]<=0:
				# {"feature": "Distance", "instances": 8, "metric_value": 0.9544, "depth": 4}
				if obj[17]<=2:
					# {"feature": "Passanger", "instances": 6, "metric_value": 0.65, "depth": 5}
					if obj[0]>0:
						return 'True'
					elif obj[0]<=0:
						return 'False'
					else: return 'False'
				elif obj[17]>2:
					return 'False'
				else: return 'False'
			else: return 'True'
		else: return 'True'
	elif obj[13]>3.0:
		return 'False'
	else: return 'False'
