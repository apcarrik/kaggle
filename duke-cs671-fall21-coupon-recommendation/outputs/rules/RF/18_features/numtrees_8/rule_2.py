def findDecision(obj): #obj[0]: Passanger, obj[1]: Weather, obj[2]: Time, obj[3]: Coupon, obj[4]: Coupon_validity, obj[5]: Gender, obj[6]: Age, obj[7]: Maritalstatus, obj[8]: Children, obj[9]: Education, obj[10]: Occupation, obj[11]: Income, obj[12]: Bar, obj[13]: Coffeehouse, obj[14]: Restaurantlessthan20, obj[15]: Restaurant20to50, obj[16]: Direction_same, obj[17]: Distance
	# {"feature": "Passanger", "instances": 127, "metric_value": 0.9838, "depth": 1}
	if obj[0]<=2:
		# {"feature": "Occupation", "instances": 100, "metric_value": 0.9997, "depth": 2}
		if obj[10]>1.6783600255305338:
			# {"feature": "Weather", "instances": 86, "metric_value": 0.9937, "depth": 3}
			if obj[1]<=0:
				# {"feature": "Coupon_validity", "instances": 65, "metric_value": 0.9985, "depth": 4}
				if obj[4]<=0:
					# {"feature": "Income", "instances": 36, "metric_value": 0.9183, "depth": 5}
					if obj[11]>0:
						# {"feature": "Maritalstatus", "instances": 32, "metric_value": 0.9544, "depth": 6}
						if obj[7]>0:
							# {"feature": "Education", "instances": 20, "metric_value": 0.8113, "depth": 7}
							if obj[9]>0:
								# {"feature": "Coffeehouse", "instances": 15, "metric_value": 0.9183, "depth": 8}
								if obj[13]<=2.0:
									# {"feature": "Coupon", "instances": 12, "metric_value": 0.9799, "depth": 9}
									if obj[3]<=2:
										# {"feature": "Time", "instances": 10, "metric_value": 0.8813, "depth": 10}
										if obj[2]<=1:
											# {"feature": "Age", "instances": 6, "metric_value": 1.0, "depth": 11}
											if obj[6]<=4:
												# {"feature": "Bar", "instances": 5, "metric_value": 0.971, "depth": 12}
												if obj[12]<=0.0:
													# {"feature": "Gender", "instances": 4, "metric_value": 0.8113, "depth": 13}
													if obj[5]<=0:
														# {"feature": "Restaurantlessthan20", "instances": 2, "metric_value": 1.0, "depth": 14}
														if obj[14]>1.0:
															return 'True'
														elif obj[14]<=1.0:
															return 'False'
														else: return 'False'
													elif obj[5]>0:
														return 'True'
													else: return 'True'
												elif obj[12]>0.0:
													return 'False'
												else: return 'False'
											elif obj[6]>4:
												return 'False'
											else: return 'False'
										elif obj[2]>1:
											return 'True'
										else: return 'True'
									elif obj[3]>2:
										return 'False'
									else: return 'False'
								elif obj[13]>2.0:
									return 'True'
								else: return 'True'
							elif obj[9]<=0:
								return 'True'
							else: return 'True'
						elif obj[7]<=0:
							# {"feature": "Restaurantlessthan20", "instances": 12, "metric_value": 0.9799, "depth": 7}
							if obj[14]<=3.0:
								# {"feature": "Bar", "instances": 9, "metric_value": 0.9911, "depth": 8}
								if obj[12]>0.0:
									# {"feature": "Coffeehouse", "instances": 5, "metric_value": 0.7219, "depth": 9}
									if obj[13]<=3.0:
										return 'True'
									elif obj[13]>3.0:
										# {"feature": "Time", "instances": 2, "metric_value": 1.0, "depth": 10}
										if obj[2]<=3:
											return 'False'
										elif obj[2]>3:
											return 'True'
										else: return 'True'
									else: return 'False'
								elif obj[12]<=0.0:
									# {"feature": "Time", "instances": 4, "metric_value": 0.8113, "depth": 9}
									if obj[2]>1:
										# {"feature": "Coupon", "instances": 2, "metric_value": 1.0, "depth": 10}
										if obj[3]>1:
											return 'True'
										elif obj[3]<=1:
											return 'False'
										else: return 'False'
									elif obj[2]<=1:
										return 'False'
									else: return 'False'
								else: return 'False'
							elif obj[14]>3.0:
								return 'False'
							else: return 'False'
						else: return 'False'
					elif obj[11]<=0:
						return 'True'
					else: return 'True'
				elif obj[4]>0:
					# {"feature": "Distance", "instances": 29, "metric_value": 0.9294, "depth": 5}
					if obj[17]>1:
						# {"feature": "Income", "instances": 18, "metric_value": 1.0, "depth": 6}
						if obj[11]>0:
							# {"feature": "Direction_same", "instances": 15, "metric_value": 0.971, "depth": 7}
							if obj[16]<=0:
								# {"feature": "Age", "instances": 12, "metric_value": 0.8113, "depth": 8}
								if obj[6]>0:
									# {"feature": "Coupon", "instances": 9, "metric_value": 0.5033, "depth": 9}
									if obj[3]>2:
										return 'False'
									elif obj[3]<=2:
										# {"feature": "Time", "instances": 2, "metric_value": 1.0, "depth": 10}
										if obj[2]<=2:
											return 'True'
										elif obj[2]>2:
											return 'False'
										else: return 'False'
									else: return 'True'
								elif obj[6]<=0:
									# {"feature": "Maritalstatus", "instances": 3, "metric_value": 0.9183, "depth": 9}
									if obj[7]>1:
										return 'True'
									elif obj[7]<=1:
										return 'False'
									else: return 'False'
								else: return 'True'
							elif obj[16]>0:
								return 'True'
							else: return 'True'
						elif obj[11]<=0:
							return 'True'
						else: return 'True'
					elif obj[17]<=1:
						# {"feature": "Bar", "instances": 11, "metric_value": 0.4395, "depth": 6}
						if obj[12]<=1.0:
							return 'False'
						elif obj[12]>1.0:
							# {"feature": "Time", "instances": 2, "metric_value": 1.0, "depth": 7}
							if obj[2]>1:
								return 'False'
							elif obj[2]<=1:
								return 'True'
							else: return 'True'
						else: return 'False'
					else: return 'False'
				else: return 'False'
			elif obj[1]>0:
				# {"feature": "Gender", "instances": 21, "metric_value": 0.7919, "depth": 4}
				if obj[5]>0:
					# {"feature": "Coupon", "instances": 13, "metric_value": 0.3912, "depth": 5}
					if obj[3]<=2:
						return 'False'
					elif obj[3]>2:
						# {"feature": "Time", "instances": 2, "metric_value": 1.0, "depth": 6}
						if obj[2]>0:
							return 'False'
						elif obj[2]<=0:
							return 'True'
						else: return 'True'
					else: return 'False'
				elif obj[5]<=0:
					# {"feature": "Maritalstatus", "instances": 8, "metric_value": 1.0, "depth": 5}
					if obj[7]<=1:
						# {"feature": "Distance", "instances": 5, "metric_value": 0.7219, "depth": 6}
						if obj[17]<=2:
							return 'False'
						elif obj[17]>2:
							return 'True'
						else: return 'True'
					elif obj[7]>1:
						return 'True'
					else: return 'True'
				else: return 'True'
			else: return 'False'
		elif obj[10]<=1.6783600255305338:
			# {"feature": "Coffeehouse", "instances": 14, "metric_value": 0.5917, "depth": 3}
			if obj[13]<=2.0:
				return 'True'
			elif obj[13]>2.0:
				# {"feature": "Coupon_validity", "instances": 4, "metric_value": 1.0, "depth": 4}
				if obj[4]>0:
					return 'False'
				elif obj[4]<=0:
					return 'True'
				else: return 'True'
			else: return 'False'
		else: return 'True'
	elif obj[0]>2:
		# {"feature": "Coupon", "instances": 27, "metric_value": 0.6913, "depth": 2}
		if obj[3]<=3:
			# {"feature": "Income", "instances": 17, "metric_value": 0.3228, "depth": 3}
			if obj[11]<=7:
				return 'True'
			elif obj[11]>7:
				return 'False'
			else: return 'False'
		elif obj[3]>3:
			# {"feature": "Children", "instances": 10, "metric_value": 0.971, "depth": 3}
			if obj[8]>0:
				# {"feature": "Restaurant20to50", "instances": 7, "metric_value": 0.5917, "depth": 4}
				if obj[15]<=2.0:
					return 'True'
				elif obj[15]>2.0:
					return 'False'
				else: return 'False'
			elif obj[8]<=0:
				return 'False'
			else: return 'False'
		else: return 'True'
	else: return 'True'
